  ...
  function runAnalysis() {
    ...
    try {
      const t = new WasmTwsc(
        ...
        undefined,
        undefined,
        undefined, // pedestrian movements 13-16, p/h
        ...
        minor_sb,
        undefined,
        undefined, // median storage NB/SB
        undefined,
        undefined, // flare storage NB/SB
      ...
      setReport({
        ...
        inputs: [
          {
            ...
            value: intersection_type === 'three_leg' ? 'Three-leg (T), minor stem northbound' : 'Four-leg',
          },
          ...
          ...(intersection_type === 'three_leg'
            ? []
            : [{ label: 'Minor lanes (SB)', value: minor_sb.replace(/_/g, ' ') }]),
          ...
          ...(intersection_type === 'three_leg'
            ? []
            : [{ label: 'Minor SB volumes L/T/R', value: `${v10} / ${v11} / ${v12} veh/h` }]),
        ...
        resultTable: {
          ...
          rows: [
            ...results.movementRows.map((m) => [
              m.name,
              fmt(m.movement_capacity, 0),
              fmt(m.vc_ratio, 2),
              fmt(m.control_delay),
              m.los ?? '',
              fmt(m.queue_95),
            ]),
            ...results.laneRows.map((l) => [
              `${l.approach} minor lane ${l.lane}`,
              fmt(l.capacity, 0),
              fmt(l.vc_ratio, 2),
              fmt(l.control_delay),
              l.los ?? '',
        ...
        summary: [
          ...results.approachRows
            ...
            .map((a) => ({ label: `Approach delay, ${a.label}`, value: `${fmt(a.delay)} s/veh` })),
...
<div class="hcm-page">
  <header class="page-header">
    ...
    <p class="page-sub">
      Estimate movement capacities, control delay, queues, and level of service for the controlled movements at a
      two-way STOP-controlled intersection, or evaluate a pedestrian crossing the major street.
    </p>
  </header>

  <div class="alert alert-info shadow-sm mb-6 beta-note" role="note">
    <span>
      The compute engine reproduces the published HCM worked examples for this chapter and applies the December 2022 HCM
      corrections, including the corrected Stage II conflicting movements and the Exhibit 20-14 swap. The pedestrian
      mode reproduces all three scenarios of Chapter 32 Example Problem 2, with the caveat noted there that two
      coefficients of Equation 20-95 are clipped out of the published text and were recovered by fitting that example.
      Verify results independently before relying on them in engineering work, and please <a
  ...
  <section class="panel">
    <div class="panel-head">
      <div>
        ...
        <p class="panel-sub">
          Section 4 evaluates the vehicular movements at the intersection. Section 5 evaluates a pedestrian crossing the
          uncontrolled major-street traffic stream, which is a separate procedure with its own service measure. The same
          Section 5 method applies to a midblock crossing.
        </p>
      </div>
    </div>
      ...
      <section class="panel">
        <div class="panel-head">
          <div>
            ...
            <p class="panel-sub">
              Legs, major-street cross section, and minor-approach lane allocation. The major street runs east-west and
              the minor stem of a three-leg intersection is northbound.
            </p>
          </div>
        </div>
        <div class="param-grid">
        ...
        <div class="diagram-block">
          <div class="diagram-head">
            <p class="panel-sub">
              Hover the legend to highlight an approach. The dash pattern shows each movement's HCM rank, and in the 2D
              view the movement volumes can be edited directly on the diagram.
            </p>
      ...
      <section class="panel">
        <div class="panel-head">
          <div>
            ...
            <p class="panel-sub">
              Turning-movement volumes numbered per HCM Exhibit 20-1. Movements 10 to 12 apply to four-leg intersections
              only.
            </p>
          </div>
        </div>
    ...
    <section class="panel results-panel">
      ...
      <div class="los overflow-x-auto">
        ...
        <div class="facility-summary">
          <p>
            LOS is reported per movement and per lane. The HCM does not define a LOS letter for a TWSC intersection as a
            whole.
          </p>
        </div>
      </div>
