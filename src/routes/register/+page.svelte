<script lang="ts">
	...
	const credentials: CredentialsRegistration = $state({
		username: '',
		email: '',
		password: '',
		first_name: '',
	...
	async function registerLocal(user: User) {
		try {
			const res = await fetch('/auth/register', {
				method: 'POST',
				body: JSON.stringify(user), // server ignores user.role - always set it to 'base' (lowest priv)
				headers: {
...
<div class="d-flex justify-content-center my-3">
	<div class="card login">
		<div class="card-body">
			...
			{#if user}
				<form id="register" autocomplete="on" novalidate class="mt-3 needs-validation">
					<div class="mb-3">
					...
					<div class="mb-3">
						...
						/>
						<div class="form-text">
							<!-- If matched, shows okay signal, if not, shows red signal -->
						</div>
					</div>
					<div class="mb-3">
