...
export function discussion(results, inputs) {
  ...
  out.push(
    failing.length > 0
      ? `${failing.length} of the ${results.od_results.length} origin-destination movements ${failing.length === 1 ? 'runs' : 'run'} at LOS E or worse, carrying ${share(
          failing.reduce((a, o) => a + (o.demand ?? 0), 0),
          totalDemand,
          0,
        )}% of the ${n(totalDemand, 0)} veh/h through the interchange, and the interchange letter is the demand-weighted average over all of them.`
