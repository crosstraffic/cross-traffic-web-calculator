<script>
  ...
  const defaultLaneGroups = () => [
    {
      movement: 'EbExtThrough',
      label: 'EB external through + right',
      lanes: 2,
      begin: 0,
      green: 63,
      is_ramp: false,
      turn_radius: null,
      shared_right_radius: 50,
      arrival: 3,
      storage: 600,
    },
    {
      movement: 'EbIntThrough',
      label: 'EB internal through',
      lanes: 2,
      begin: 116,
      green: 97,
      is_ramp: false,
      turn_radius: null,
      shared_right_radius: null,
      arrival: 4,
      storage: 500,
    },
    {
      movement: 'EbIntLeft',
      label: 'EB internal left (to NB on-ramp)',
      lanes: 1,
      begin: 116,
      green: 29,
      is_ramp: false,
    ...
    {
      movement: 'WbExtThrough',
      label: 'WB external through + right',
      lanes: 2,
      begin: 150,
      green: 63,
      is_ramp: false,
    ...
    {
      movement: 'WbIntThrough',
      label: 'WB internal through',
      lanes: 2,
      begin: 0,
      green: 111,
      is_ramp: false,
    ...
    {
      movement: 'WbIntLeft',
      label: 'WB internal left (to SB on-ramp)',
      lanes: 1,
      begin: 68,
      green: 43,
      is_ramp: false,
    ...
    {
      movement: 'NbRampRight',
      label: 'NB off-ramp right',
      lanes: 1,
      begin: 58,
      green: 53,
      is_ramp: true,
    ...
    {
      movement: 'SbRampRight',
      label: 'SB off-ramp right',
      lanes: 1,
      begin: 116,
      green: 39,
      is_ramp: true,
  ...
  const parcloLaneGroups = () => [
    ...
    {
      movement: 'EbExtLeft',
      label: 'EB external left (to SB on-ramp)',
      lanes: 1,
      begin: 0,
      green: 25,
      is_ramp: false,
    ...
    {
      movement: 'EbIntThroughRight',
      label: 'EB internal through + right (to NB loop)',
      lanes: 3,
      begin: 70,
      green: 65,
      is_ramp: false,
    ...
    {
      movement: 'WbExtLeft',
      label: 'WB external left (to NB on-ramp)',
      lanes: 1,
      begin: 0,
      green: 25,
      is_ramp: false,
    ...
    {
      movement: 'WbIntThroughRight',
      label: 'WB internal through + right (to SB loop)',
      lanes: 3,
      begin: 30,
      green: 60,
      is_ramp: false,
  ...
  const spuiLaneGroups = () => [
    {
      movement: 'EbExtLeft',
      label: 'EB arterial left (to NB on-ramp)',
      lanes: 1,
      begin: 0,
      green: 16,
      is_ramp: false,
      ...
      grade: 2,
      width: 10.3,
      perm: { begin: 24, green: 32, gu: 13.01 },
    },
    {
      movement: 'EbExtThrough',
      label: 'EB arterial through',
      lanes: 2,
      begin: 24,
      green: 32,
      is_ramp: false,
    ...
    {
      movement: 'EbExtRight',
      label: 'EB arterial right (to SB on-ramp)',
      lanes: 1,
      begin: 64,
      green: 38,
      is_ramp: false,
    ...
    {
      movement: 'WbExtLeft',
      label: 'WB arterial left (to SB on-ramp)',
      lanes: 1,
      begin: 0,
      green: 16,
      is_ramp: false,
      ...
      grade: 2,
      width: 10.3,
      perm: { begin: 24, green: 32, gu: 11.78 },
    },
    {
      movement: 'WbExtThrough',
      label: 'WB arterial through',
      lanes: 2,
      begin: 24,
      green: 32,
      is_ramp: false,
    ...
    {
      movement: 'WbExtRight',
      label: 'WB arterial right (to NB on-ramp)',
      lanes: 1,
      begin: 64,
      green: 38,
      is_ramp: false,
  ...
  let losByOd = $derived(
    results ? Object.fromEntries(results.od_results.filter((o) => o.los).map((o) => [o.movement, o.los])) : {},
  );
  ...
  function runAnalysis() {
    ...
    try {
      ...
      setReport({
        ...
        inputs: [
          {
            label: 'Interchange form',
            value: `${FORM_LABEL[form][0].toUpperCase()}${FORM_LABEL[form].slice(1)}, pretimed signals`,
          },
          ...
          {
            label: 'O-D demands (A-N)',
            value: odDemands.map((d) => `${d.key.toUpperCase()} ${d.value}`).join(', ') + ' veh/h',
          },
        ],
        resultTable: {
...
<div class="hcm-page">
  <header class="page-header">
    ...
    <p class="page-sub">
      Estimate experienced travel time and level of service by origin-destination movement and for the facility as a
      whole, for signalized diamond interchanges (Part B) and for RCUT, MUT, and DLT alternative intersections (Part C).
    </p>
  </header>

  <div class="alert alert-info shadow-sm mb-6 beta-note" role="note">
    <span>
      The compute engine reproduces the published HCM Chapter 34 example problems within the library's documented
      tolerances: the conventional diamond, the diverging diamond, the Parclo A-2Q, and the single-point urban
      interchange with pretimed signals under Part B, and the STOP-controlled RCUT, the MUT, and the DLT evaluations
      under Part C. The other five partial cloverleaf forms of Exhibit 23-17 (A-4Q, AB-2Q, AB-4Q, B-2Q, B-4Q) are
      supported by the engine but have no published example problem behind them, so they are not offered here. Verify
      results independently before relying on them in engineering work, and please <a
  ...
  <section class="panel">
    <div class="panel-head">
      <div>
        ...
        <p class="panel-sub">
          Part B evaluates signalized interchange ramp terminals. Part C evaluates alternative intersections: the
          restricted crossing U-turn, median U-turn, and displaced left-turn.
        </p>
      </div>
    </div>
  ...
  {:else}
    ...
    {#if form === 'Spui'}
      <div class="alert alert-info shadow-sm mb-6 spui-note" role="note">
        <span>
          Defaults are HCM Chapter 34, Example Problem 7, the SPUI at I-95 and University Drive (Exhibits 34-72 through
          34-80). It is the one worked example on this page whose saturation flow worksheets were carried over from a
          superseded edition, and seven documented defects follow from that. Exhibits 34-75 and 34-76 print a lane width
          factor of 0.967 for the stated 10.3 ft lanes, which is the HCM 2000 continuous form and which HCM 7 Exhibit
          19-20 cannot produce for any width from 10.0 to 12.9 ft; Exhibit 34-76 then prints a heavy vehicle and grade
          factor of 1.000 on the two ramp approaches where the example's own 5% heavy vehicles give 0.961; the traffic
          pressure row reproduces Equation 23-15 exactly for the eight columns that are not a left-turn phase component
          and for none of the four that are, and it prints different values for the protected and permitted halves of a
          single movement at a single demand, which the equation cannot do. Exhibit 34-77 then contradicts itself three
          ways: its own g<sub>u</sub> = G<sub>perm</sub> − g<sub>q</sub> − l<sub>1</sub>
          relation closes eastbound and misses westbound by 1.8 s, it prints a westbound uniform delay of 22.7 s/veh where
          Exhibit 34-80's own sum needs 22.8, and it labels its queue breakpoints in feet where Chapter 31 defines every
          one of them in vehicles. Exhibit 34-78 finally omits the Equation 31-124 sneaker term from the left-turn capacity,
          worth about 65 veh/h, which the engine omits too because Chapter 34 does. Eight of the ten published O-D LOS letters
          and the interchange LOS still reproduce. The two that differ, D and E, are the two sitting closest to an Exhibit
          23-10 band edge.
        </span>
      </div>
    {/if}
      ...
      <section class="panel">
        <div class="panel-head">
          <div>
            ...
            <p class="panel-sub">
              Signal timing and geometry shared by both ramp terminal intersections. The arterial runs east-west, with
              the southbound ramps at the west intersection and the northbound ramps at the east intersection.
            </p>
          </div>
        </div>
        <div class="param-grid">
          ...
          {#if form === 'ParcloA2Q'}
            ...
            <div class="param-field">
              ...
              <p class="param-hint">
                Equation 23-50 takes a design speed per diverted movement, so the loops need not match the rest.
              </p>
            </div>
          {/if}
      ...
      <section class="panel">
        <div class="panel-head">
          <div>
            ...
            <p class="panel-sub">
              O-D movements per Exhibit 23-8. Hover the legend to isolate a group; demands are editable on the 2D
              picture, and the traffic animation slows per O-D LOS after a run.
            </p>
          </div>
      ...
      <section class="panel">
        <div class="panel-head">
          <div>
            ...
            <p class="panel-sub">
              Hourly demand volumes for the fourteen O-D movements of HCM Exhibit 23-20. Frontage-road and U-turn
              movements are usually 0.
            </p>
          </div>
        </div>
        <div class="param-grid">
      ...
      <section class="panel">
        <div class="panel-head">
          <div>
            ...
            <p class="panel-sub">
              Lanes and displayed green interval for each interchange lane group. Green begin times are measured from
              the start of the common cycle and may wrap past the end of the cycle.{hasPermitted
                ? ' The two arterial left turns run twice per cycle, protected in the first green window and permitted in the second, so they carry a second window and the part of it the opposing queue leaves unblocked.'
                : ''}
            </p>
          </div>
        </div>
            ...
            <tbody>
              {#each laneGroups as g (g.movement)}
                <tr>
                  ...
                  <td>
                    <input
                      id="LG_{g.movement}_lanes"
                      aria-label="{g.label} lanes"
                      type="number"
                      min="1"
                      max="4"
                      class="input input-bordered input-sm"
                  ...
                  <td>
                    <input
                      id="LG_{g.movement}_begin"
                      aria-label="{g.label} green begin"
                      type="number"
                      step="0.1"
                      min="0"
                      class="input input-bordered input-sm"
                  ...
                  <td>
                    <input
                      id="LG_{g.movement}_green"
                      aria-label="{g.label} green duration"
                      type="number"
                      step="0.1"
                      min="1"
                      class="input input-bordered input-sm"
                  ...
                  {#if hasPermitted}
                    {#if g.perm}
                      <td>
                        <input
                          id="LG_{g.movement}_permbegin"
                          aria-label="{g.label} permitted green begin"
                          type="number"
                          step="0.1"
                          min="0"
                          class="input input-bordered input-sm"
                          bind:value={g.perm.begin}
                          required
                        />
                      </td>
                      <td>
                        <input
                          id="LG_{g.movement}_permgreen"
                          aria-label="{g.label} permitted green duration"
                          type="number"
                          step="0.1"
                          min="1"
                          class="input input-bordered input-sm"
                          bind:value={g.perm.green}
                          required
                        />
                      </td>
                      <td>
                        <input
                          id="LG_{g.movement}_gu"
                          aria-label="{g.label} unblocked permitted green"
                          type="number"
                          step="0.01"
                          min="0"
                          class="input input-bordered input-sm"
                          bind:value={g.perm.gu}
                          required
                        />
                      </td>
                    {:else}
        ...
        {#if hasPermitted}
          <p class="panel-sub">
            g<sub>u</sub> is the part of the permitted green the opposing queue leaves unblocked (Equation 31-95),
            carried as an input because Exhibit 34-77 publishes it directly and its own g<sub>q</sub> row does not reproduce
            it westbound. The opposing flow follows the other direction's through demand, so it is not entered separately.
          </p>
        {/if}
      </section>
