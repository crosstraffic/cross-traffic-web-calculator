<script>
  ...
  function decideConsent(choice) {
    consent = choice;
    try {
      localStorage.setItem('hcm-analytics-consent', choice);
    } catch (e) {
      /* private mode */
    }
...
<header>
  <div class="navbar bg-base-100 shadow-md">
    <div class="navbar-start">
      ...
      <!-- Gonna be LOGO -->
      <a href="/" class="normal-case text-xl logo"
        ><img src="hcm_calculator_logo.png" alt="logo" style="max-width:110px;height:100%" /></a
      >
    </div>
...
{#if analyticsEligible && consent === 'unset'}
  <div class="consent-banner" role="dialog" aria-label="Analytics consent">
    <p class="consent-text">
      This site would like to use Google Analytics to count visits and see which chapters get used. Nothing loads unless
      you accept, declining changes nothing about the calculator, and your analysis data never leaves your browser
      either way.
      <a href="/terms">Details</a>
    </p>
    <div class="consent-actions">
      <button type="button" class="btn btn-sm btn-primary" onclick={() => decideConsent('granted')}
