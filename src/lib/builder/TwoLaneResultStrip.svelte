...
<section class="ts-wrap" aria-label="Segment results" data-testid="twolane-result-strip">
  ...
  <p class="ts-single" data-testid="twolane-single-period">
    The Chapter 15 method is single-period, so this is one value per segment rather than a time-space grid. Chapter 15
    also has no reliability methodology, so unlike a freeway or an urban street facility there is no distribution below
    this to hand the highway to.
  </p>
          ...
          {#each result.segments as s, i}
            ...
            <td
              ...
              aria-label="segment {i + 1}, {measure.label} {cellText(measure, v)}{measure.unit
                ? ` ${measure.unit}`
                : ''}{measure.id === 'followerDensity' && s.passingType === 2
                ? ', midpoint value of a passing lane'
                : ''}"
  ...
  {#if measure.id === 'followerDensity' && result.segments.some((s) => s.passingType === 2)}
    <p class="ts-note" data-testid="twolane-midpoint-note">
      The outlined cell is a passing lane, and its value is the midpoint follower density of Step 8 rather than the
      endpoint value the other cells hold. Steps 10 and 11 read the midpoint for a passing lane, so it is the comparable
      number for the letter and for the facility, but it is not the density a driver leaving the lane meets.
    </p>
  {/if}
  ...
  {#if detail}
    <div class="ts-detail" data-testid="twolane-cell-detail">
      ...
      <dl>
        <div>
          <dt>Type</dt>
          <dd>{detail.segType}</dd>
        </div>
        <div>
          <dt>Length</dt>
          <dd>{mi(detail.lengthMi)} mi</dd>
        </div>
        <div>
          <dt>Grade</dt>
          <dd>{n(detail.grade, 1)} %</dd>
        </div>
        <div>
          <dt>Posted limit</dt>
          <dd>{n(detail.spl, 0)} mi/h</dd>
        </div>
        <div>
          <dt>Vertical class</dt>
          <dd>
            {detail.verticalAlignment}{detail.verticalAlignment !== detail.verticalClassEntered
              ? ` (entered ${detail.verticalClassEntered})`
              : ''}
          </dd>
        </div>
        <div>
          <dt>Demand flow</dt>
          <dd>{n(detail.flowRate, 0)} veh/h</dd>
        </div>
        <div>
          <dt>Opposing flow</dt>
          <dd>{n(detail.opposingFlow, 0)} veh/h</dd>
        </div>
        <div>
          <dt>Capacity</dt>
          <dd>{n(detail.capacity, 0)} veh/h</dd>
        </div>
        <div>
          <dt>Free-flow speed</dt>
          <dd>{n(detail.ffs, 2)} mi/h</dd>
        </div>
        <div>
          <dt>Average speed</dt>
          <dd>{n(detail.avgSpeed, 1)} mi/h</dd>
        </div>
        <div>
          <dt>Percent followers</dt>
          <dd>{n(detail.percentFollowers, 1)} %</dd>
        </div>
        <div>
          <dt>Follower density</dt>
          <dd>{n(detail.followerDensity, 2)} followers/mi</dd>
        </div>
        {#if detail.passingType === 2}
          <div>
            <dt>At its end</dt>
            <dd>{n(detail.fd, 2)} followers/mi</dd>
          </div>
        {:else if detail.fdAdjustment > 0}
          <div>
            <dt>Unadjusted</dt>
            <dd>{n(detail.fd, 2)} followers/mi, before the Step 9 passing-lane adjustment</dd>
          </div>
        {/if}
        <div>
          <dt>LOS</dt>
        ...
        {#if detail.isHc}
          <div>
            <dt>Horizontal curves</dt>
            <dd>{detail.curveCount} in {detail.subsegmentCount} subsegments (Step 5d)</dd>
          </div>
        {/if}
        {#if detail.demotedPassingLane}
          <div>
            <dt>Passing lane</dt>
            <dd>Placed here but shorter than the 0.5 mi Exhibit 15-10 minimum, so analyzed as Passing Constrained</dd>
          </div>
        {/if}
        {#if detail.outsideRecommended}
          <div>
            <dt>Exhibit 15-10</dt>
            <dd>
              Outside the recommended {n(detail.minLengthMi, 2)} to {n(detail.maxLengthMi, 2)} mi for this class and type
            </dd>
          </div>
        {/if}
        {#if detail.overridden}
          <div>
            <dt>Override</dt>
            <dd>This row was pinned by hand, so it does not follow the features.</dd>
          </div>
        {/if}
      </dl>
    </div>
