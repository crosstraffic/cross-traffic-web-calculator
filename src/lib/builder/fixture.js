...
export function fromUrbanFixture(raw, name = 'imported fixture') {
  ...
  doc.analysisMode = raw.segments.some((s) => URBAN_MEASURE_KEYS.some((k) => s[k] != null)) ? 'measures' : 'inputs';
  ...
  raw.segments.forEach((s, i) => {
    ...
    addPoints(
      doc,
      segStart,
      s.segment_length_ft ?? 0,
      i,
      'subject',
      Math.round(s.n_access_points_subject ?? 0),
      delays,
      approaches,
    );
    addPoints(
      ...
      i,
      'opposing',
      Math.round(s.n_access_points_opposing ?? 0),
      [],
      [],
    );
  });
...
export function toTwoLaneFixture(doc, rows) {
  if (doc.importedRaw) {
    ...
    out.segments = out.segments.map((orig, i) =>
      rows[i] ? mergeSegment(orig, rows[i], TWOLANE_SEGMENT_KEYS, (r, k) => !same(r[k], baseline[i]?.[k])) : orig,
    );
    return out;
  }
