...
<div class="fe" data-testid="feature-editor" data-feature-id={feature.id} data-kind={feature.kind}>
  ...
  <div class="fe-grid">
    <label>Label
      <input type="text" value={feature.label} placeholder={feature.id} disabled={!interactive}
    ...
    {#if feature.kind === 'work_zone'}
      <label>Begins (mi)
        <input type="number" min="0" step="0.01" value={(feature.stationFt / FT_PER_MI).toFixed(2)}
      ...
      <label>Ends (mi)
        <input type="number" min="0" step="0.01" value={(feature.endFt / FT_PER_MI).toFixed(2)}
               disabled={!interactive} data-testid="end-{feature.id}"
    ...
    {#if feature.kind === 'on_ramp' || feature.kind === 'off_ramp'}
      <label>Ramp FFS (mi/h)
        <input type="number" min="15" max="70" step="1" value={feature.rampFfs} disabled={!interactive}
      ...
      {#if feature.kind === 'on_ramp'}
        <label>Acceleration lane (ft)
          <input type="number" min="0" step="10" value={feature.accelLaneFt} disabled={!interactive}
      ...
      {:else}
        <label>Deceleration lane (ft)
          <input type="number" min="0" step="10" value={feature.decelLaneFt} disabled={!interactive}
    ...
    {#if feature.kind === 'lane_change'}
      <label>Lanes downstream
        <input type="number" min="2" max="8" step="1" value={feature.lanes} disabled={!interactive}
  ...
  {#if feature.kind === 'on_ramp'}
    <div class="fe-sub">
      <label class="fe-check">
        <input type="checkbox" checked={feature.auxLaneToNext} disabled={!interactive}
      ...
      <p class="fe-note">
        An auxiliary lane running from this gore to the next off-ramp's makes the whole section one weaving segment (Exhibit 10-12), one lane wider than the mainline. The three fields below are read only while it does.
      </p>
      <div class="fe-grid" class:fe-off={!feature.auxLaneToNext}>
        <label>Weaving lanes
          <input type="number" min="2" max="4" step="1" value={feature.numWeavingLanes} disabled={!interactive}
        ...
        <label>Min lane changes, ramp to freeway
          <input type="number" min="0" max="3" step="1" value={feature.lcRf} disabled={!interactive}
        ...
        <label>Min lane changes, freeway to ramp
          <input type="number" min="0" max="3" step="1" value={feature.lcFr} disabled={!interactive}
  ...
  {#if feature.kind === 'work_zone'}
    <div class="fe-sub">
      <p class="fe-note">
        The engine analyzes the lanes that stay open. The lanes the closure takes out feed the severity index instead, which is why both counts are coded (Chapter 10 Section 4, Equations 10-7 through 10-12).
      </p>
      <div class="fe-grid">
        <label>Lanes, total
          <input type="number" min="1" max="8" step="1" value={feature.config.total_lanes} disabled={!interactive}
        ...
        <label>Lanes, open
          <input type="number" min="1" max="8" step="1" value={feature.config.open_lanes} disabled={!interactive}
        ...
        <label>Barrier
          <select value={feature.config.soft_barrier ? 'soft' : 'hard'} disabled={!interactive}
        ...
        <label>Speed limit (mi/h)
          <input type="number" min="25" max="75" step="5" value={feature.config.speed_limit_mi_h} disabled={!interactive}
        ...
        <label>Speed ratio
          <input type="number" min="0.5" max="2" step="0.0001" value={feature.config.speed_ratio} disabled={!interactive}
        ...
        <label>Queue discharge drop
          <input type="number" min="0" max="0.5" step="0.001" value={feature.config.queue_discharge_drop} disabled={!interactive}
        ...
        <label>Lateral distance (ft)
          <input type="number" min="0" step="1" value={feature.config.lateral_distance_ft} disabled={!interactive}
        ...
        <!-- The mainline carries its own ramp density and the work zone's copy
             ...
             facility average. -->
        <label>Ramp density here (/mi)
          <input type="number" min="0" step="0.1" value={feature.config.total_ramp_density} disabled={!interactive}
        ...
        <label class="fe-check">
          <input type="checkbox" checked={feature.config.night} disabled={!interactive}
        ...
        <label class="fe-check">
          <input type="checkbox" checked={feature.config.rural} disabled={!interactive}
    ...
    <div class="fe-sub">
      ...
      <div class="fe-periods" data-testid="feature-demands-{feature.id}">
        {#each Array.from({ length: periods }) as _, p}
          <label>P{p + 1}
            <input type="number" min="0" step="1" value={feature.demand[p] ?? 0} disabled={!interactive}
      ...
      {#if feature.kind === 'on_ramp' && feature.auxLaneToNext}
        ...
        <div class="fe-periods" data-testid="feature-r2r-{feature.id}">
          {#each Array.from({ length: periods }) as _, p}
            <label>P{p + 1}
              <input type="number" min="0" step="1" value={feature.rampToRampDemand[p] ?? 0} disabled={!interactive}
...
<style>
  ...
  .fe-head { display: flex; align-items: baseline; gap: 0.5rem; margin-bottom: 0.4rem; }
  .fe-head h4 { margin: 0; font-size: 0.85rem; }
  .fe-id { font-size: 0.7rem; color: var(--text-faint); font-family: ui-monospace, monospace; }

  .fe-grid { display: flex; flex-wrap: wrap; gap: 0.45rem 1rem; }
  .fe-grid label { font-size: 0.72rem; color: var(--text-secondary); display: inline-flex; flex-direction: column; gap: 0.1rem; }
  .fe-grid label.fe-check, .fe-check { flex-direction: row; align-items: center; gap: 0.3rem; }
  ...
  .fe-sub { margin-top: 0.6rem; padding-top: 0.5rem; border-top: 1px dashed var(--border); }
  .fe-sub h5 { margin: 0 0 0.25rem; font-size: 0.74rem; color: var(--text-secondary); font-weight: 600; }
  .fe-sub h5 + .fe-periods { margin-bottom: 0.5rem; }
  .fe-note { font-size: 0.72rem; color: var(--text-muted); margin: 0.15rem 0 0.4rem; line-height: 1.5; max-width: 82ch; }

  .fe-periods { display: flex; flex-wrap: wrap; gap: 0.3rem 0.5rem; }
  .fe-periods label { font-size: 0.68rem; color: var(--text-muted); display: inline-flex; flex-direction: column; gap: 0.08rem; }
