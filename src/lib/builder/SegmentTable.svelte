...
<div class="st-wrap">
  <div class="st-head">
    ...
    <p class="st-sub">
      {rows.length} segment{rows.length === 1 ? '' : 's'}, {totalMi.toFixed(2)} mi{overrideCount
      ...
      <tbody>
        {#each rows as r, i (r.key)}
          <tr
            ...
            class:overridden={r.overridden}
            data-testid="segment-row"
            data-seg-key={r.key}
            data-seg-type={r.seg_type}
          >
            <th scope="row">
              <button
                type="button"
            ...
            <td>
              {#if interactive}
                <select
                  value={r.seg_type}
                  onchange={(e) => edit(r, 'seg_type', e.currentTarget.value)}
            ...
            <td class="st-flags">
              {#if r.overridden}
                ...
                {#if r.staleOverride}
                  <span
                    class="st-stale"
                    data-testid="override-stale"
                    title="The derivation now produces a {r.derivedSegType} here, so this override was made against a different segment."
                    >stale</span
                  >
                {/if}
                <button
          ...
          {#if selectedKey === r.key}
            <tr class="st-why" data-testid="why-row">
              <td colspan="7">
                ...
                {#if r.overridden}
                  <br /><em
                    >This row is pinned. The derivation produces {r.derivedSegType ?? r.seg_type} at {Math.round(
                      r.derivedLengthFt ?? r.length_ft,
                    )} ft; clearing the override restores that.</em
                  >
                {/if}
              </td>
            </tr>
  ...
  {#if doc?.importedSegments}
    <p class="st-imported" data-testid="imported-note">
      This facility was imported from a fixture, which stores segments rather than the ramps that produced them. There
      is no feature layer to re-derive from, so this table is the editor and every change here is an override.
    </p>
  {/if}
</div>
...
<style>
  ...
  .st-table tr.selected > * {
    background: var(--accent-soft);
  }
  .st-table tr.overridden > * {
