<script>
  ...
  let TOP = $derived(twolane ? LANE + Math.max(TOP_FREEWAY - LANE, tlRows + 28) : urban ? TOP_URBAN : TOP_FREEWAY);
  ...
  let laneChanges = $derived(
    (doc?.features ?? []).filter((f) => f.kind === 'lane_change').map((f) => ({ f, x: xOf(f.stationFt) })),
  );
  let zones = $derived(
    (doc?.features ?? [])
...
<div class="bs-strip" data-testid="builder-strip">
  ...
  >
    ...
    {#each laid as s (s.r.key)}
      ...
      <g
        class="bs-seg"
        class:selected={selectedKey === s.r.key}
        class:overridden={s.r.overridden}
        data-testid="strip-seg"
        data-seg-type={s.r.seg_type}
        data-seg-key={s.r.key}
        data-seg-lanes={s.open}
        data-seg-wz={s.wz ? 'yes' : null}
        onclick={() => pick(s.r)}
        role="button"
        tabindex="-1"
        aria-label="segment {s.i + 1}, {s.r.seg_type}, {Math.round(s.r.length_ft)} feet, {s.open} lanes{s.wz
          ? `, work zone ${s.wz.total} to ${s.wz.open} lanes`
          : ''}"
      >
        <title
          >Segment {s.i + 1} · {s.r.seg_type} · {Math.round(s.r.length_ft).toLocaleString('en-US')} ft · station {mi(
            s.r.startFt,
          )}–{mi(s.r.startFt + s.r.length_ft)} mi</title
        >
        <rect
          x={s.x}
          y={TOP}
          ...
          height={s.open * LANE}
          fill={twolane
            ? (FILL_TL[s.r.seg_type] ?? FILL_TL['Passing Constrained'])
            : (FILL[s.r.seg_type] ?? FILL.Basic)}
          class="bs-main"
        />
        ...
        {#if s.w > 34}
          ...
          <text
            x={s.mid}
            y={TOP + (Math.floor(s.open / 2) + 0.5) * LANE + 3}
            class="bs-seg-label"
            class:plain={urban || s.r.seg_type === 'Basic'}
            text-anchor="middle"
            >{urban
              ? `${Math.round(s.r.length_ft).toLocaleString('en-US')} ft`
              : twolane
                ? (SHORT_TL[s.r.seg_type] ?? s.r.seg_type)
                : (SHORT[s.r.seg_type] ?? s.r.seg_type)}</text
          >
        {/if}
    ...
    {#each features as { f, x } (f.id)}
      ...
      <g
        class="bs-feat"
        class:on={f.kind === 'on_ramp'}
        class:dragging={dragging === f.id}
        class:lit
        data-testid="feature-marker"
        data-feature-id={f.id}
        data-station-ft={f.stationFt}
        role="slider"
        tabindex="0"
        aria-label="{f.kind === 'on_ramp' ? 'on' : 'off'}-ramp {f.label || f.id} at station {mi(f.stationFt)} mi"
        aria-valuemin="0"
        aria-valuemax={Math.round(L)}
        aria-valuenow={f.stationFt}
        aria-valuetext="{mi(f.stationFt)} mi"
      ...
      >
        ...
        {#if f.kind === 'on_ramp'}
          <polygon
            points="{x - 16},{bot + RAMP_H} {x - 7},{bot + RAMP_H} {x},{bot + 4} {x - 6},{bot + 4}"
            class="bs-ramp"
          />
        {:else}
          <polygon
    ...
    {#each signals as { f, x } (f.id)}
      ...
      >
        <title
          >Signal {f.label || f.id} · station {mi(f.stationFt)} mi · C {f.config?.cycle_length_s ?? '–'} s, g {f.config
            ?.effective_green_s ?? '–'} s</title
        >
    ...
    {#each accessPoints as { f, x } (f.id)}
      ...
      <g
        ...
        class:opposing={opp}
        class:dragging={dragging === f.id}
        data-testid="access-point-marker"
        data-feature-id={f.id}
        data-station-ft={f.stationFt}
        data-side={f.side}
        data-source={f.delayS != null ? 'published' : f.approach ? 'computed' : 'planning'}
        role="slider"
        tabindex="0"
      ...
      >
        <title
          >{opp ? 'Opposing' : 'Subject'}-side access point {f.label || f.id} · station {mi(f.stationFt)} mi{f.delayS !=
          null
            ? ` · ${f.delayS} s/veh supplied`
            : f.approach
              ? ' · approach supplied'
              : ''}</title
        >
    ...
    {#each laneChanges as { f, x } (f.id)}
      ...
      <g
        class="bs-lc"
        class:lit
        data-testid="lane-change-marker"
        data-feature-id={f.id}
        data-station-ft={f.stationFt}
        data-lanes={f.lanes}
        role="slider"
      ...
      >
        ...
        <text {x} y={TOP - 6} class="bs-lc-chip" text-anchor="middle">{f.lanes} ln</text>
      </g>
    {/each}
    ...
    {#each zones as { f, x, w } (f.id)}
      ...
      <g
        class="bs-zone"
        class:lit
        data-testid="work-zone-marker"
        data-feature-id={f.id}
        data-station-ft={f.stationFt}
        data-end-ft={f.endFt}
        role="slider"
        tabindex="0"
        aria-label="work zone from {mi(f.stationFt)} to {mi(f.endFt)} mi, {f.config.total_lanes} lanes to {f.config
          .open_lanes}"
        aria-valuemin="0"
        aria-valuemax={Math.round(L)}
        aria-valuenow={f.stationFt}
      ...
      >
        <title
          >Work zone {mi(f.stationFt)}&ndash;{mi(f.endFt)} mi, {f.config.total_lanes} lanes to {f.config
            .open_lanes}</title
        >
        ...
        <text x={x + w / 2} y={TOP - 23} class="bs-zone-chip" text-anchor="middle"
          >WZ {f.config.total_lanes}&rarr;{f.config.open_lanes}</text
        >
      </g>
    {/each}
    ...
    {#each gradeSpans as { f, x, w } (f.id)}
      ...
      <g
        class="bs-grade"
        class:lit
        class:down={f.gradePct < 0}
        class:dragging={dragging === f.id}
        data-testid="grade-marker"
        data-feature-id={f.id}
        data-station-ft={f.stationFt}
        data-end-ft={f.endFt}
        data-grade={f.gradePct}
        data-vertical-class={f.verticalClass}
        role="slider"
        tabindex="0"
        aria-label="{f.gradePct > 0 ? 'upgrade' : 'downgrade'} of {Math.abs(
          f.gradePct,
        )} percent at vertical class {f.verticalClass}, from {mi(f.stationFt)} to {mi(f.endFt)} mi"
        aria-valuemin="0"
        aria-valuemax={Math.round(L)}
        aria-valuenow={f.stationFt}
        aria-valuetext="{mi(f.stationFt)} mi"
      ...
      >
        <title
          >{f.gradePct > 0 ? '+' : ''}{f.gradePct}% grade, vertical class {f.verticalClass} &middot; {mi(
            f.stationFt,
          )}&ndash;{mi(f.endFt)} mi</title
        >
        ...
        <line
          x1={x + w}
          y1={rulerY - TL_ROW.grade - 4}
          x2={x + w}
          y2={rulerY - TL_ROW.grade + 4}
          class="bs-grade-bar"
        />
        <text x={x + w / 2} y={rulerY - TL_ROW.grade - 3} class="bs-grade-chip" text-anchor="middle"
          >{f.gradePct > 0 ? '+' : ''}{f.gradePct}% &middot; VC{f.verticalClass}</text
        >
      </g>
    {/each}
    ...
    {#each passingSpans as { f, x, w } (f.id)}
      ...
      <g
        ...
        class:lane
        class:dragging={dragging === f.id}
        data-testid="passing-marker"
        data-feature-id={f.id}
        data-station-ft={f.stationFt}
        data-end-ft={f.endFt}
        data-passing-type={f.passingType}
        role="slider"
        tabindex="0"
      ...
      >
        ...
        <text x={x + w / 2} y={rulerY - TL_ROW.pass - 3} class="bs-pass-chip" text-anchor="middle"
          >{lane ? 'PL' : 'PZ'}</text
        >
    ...
    {#each curveSpans as { f, x, w } (f.id)}
      ...
      <g
        ...
        class:lit
        class:dragging={dragging === f.id}
        data-testid="curve-marker"
        data-feature-id={f.id}
        data-station-ft={f.stationFt}
        data-end-ft={f.endFt}
        data-radius-ft={f.designRadiusFt}
        data-superelevation={f.superelevationPct}
        role="slider"
        tabindex="0"
        aria-label="horizontal curve of {Math.round(
          f.designRadiusFt,
        )} ft radius at {f.superelevationPct} percent superelevation, from {mi(f.stationFt)} to {mi(f.endFt)} mi"
        aria-valuemin="0"
        aria-valuemax={Math.round(L)}
        aria-valuenow={f.stationFt}
        aria-valuetext="{mi(f.stationFt)} mi"
      ...
      >
        <title
          >Horizontal curve &middot; {Math.round(f.designRadiusFt)} ft radius, {f.superelevationPct}% superelevation
          &middot; {Math.round(f.endFt - f.stationFt)} ft</title
        >
        ...
        <text x={x + w / 2} y={bot + 8 + curveBow(f)} class="bs-curve-chip" text-anchor="middle"
          >R{Math.round(f.designRadiusFt)}</text
        >
    ...
    {#each demandMarks as { f, x } (f.id)}
      ...
      <g
        ...
        class:lit
        class:dragging={dragging === f.id}
        data-testid="demand-marker"
        data-feature-id={f.id}
        data-station-ft={f.stationFt}
        data-volume={f.config?.volume}
        role="slider"
        tabindex="0"
        aria-label="demand change to {Math.round(f.config?.volume ?? 0)} vehicles per hour at station {mi(
          f.stationFt,
        )} mi"
        aria-valuemin="0"
        aria-valuemax={Math.round(L)}
      ...
      >
        ...
        <text {x} y={rulerY - TL_ROW.demand} class="bs-demand-chip" text-anchor="middle"
          >{Math.round(f.config?.volume ?? 0)}</text
        >
  ...
  {#if twolane}
    <p class="bs-note">
      Segments break where the grade, the passing type, the demand or the posted limit changes, and cannot be dragged
      themselves. Drag a marker, or focus it and use the arrow keys, to move the feature that produced a boundary;
      stations snap to 0.1 mi and the station field is the fine adjustment. The centerline above the pavement is solid
      where passing is not permitted and dashed through a passing zone, and a passing lane is drawn as the added lane it
      is. A curve hangs below the pavement because it does NOT break a segment: it becomes a subsegment of whichever
      segment contains it, in feet. Clicking a marker without moving it opens that feature's editor in the list below.
    </p>
  {:else if urban}
    <p class="bs-note">
      Segments run from one boundary signal to the next and cannot be dragged themselves. Drag a signal, or focus it and
      use the arrow keys, to move a boundary; stations snap to 0.1 mi and the station field is the fine adjustment. Each
      segment reads its timing from the signal at its downstream end, so moving a signal changes the segment before it
      as well as the one after. Access points hang below the pavement on the subject side and above it on the opposing
      side, at the stations they were placed at. Clicking a marker without moving it opens that feature's editor in the
      list below.
    </p>
  {:else}
    <p class="bs-note">
      Segments are derived from the features by the Chapter 10 rules and cannot be dragged. Drag a ramp marker, or focus
      it and use the arrow keys, to move its gore; stations snap to 0.1 mi and the station field is the fine adjustment.
      Clicking a marker without moving it opens that feature's editor in the list below.
    </p>
  {/if}
</div>
...
<style>
  ...
  .bs-seg.selected .bs-main {
    stroke: var(--accent);
    stroke-width: 2.5;
  }
  .bs-seg.overridden .bs-main {
  ...
  .bs-feat.on .bs-handle {
    stroke: var(--accent);
  }
  .bs-feat.lit .bs-handle,
  .bs-feat.dragging .bs-handle {
    fill: var(--accent);
    stroke: var(--accent-strong);
  }
  .bs-feat.lit .bs-gore,
  .bs-feat.dragging .bs-gore {
    stroke: var(--accent);
    stroke-dasharray: none;
  }
  .bs-feat:focus-visible .bs-handle {
  ...
  .bs-sig.lit .bs-handle,
  .bs-sig.dragging .bs-handle {
    fill: var(--accent);
    stroke: var(--accent-strong);
  }
  .bs-sig:focus-visible .bs-handle {
    fill: var(--accent-soft);
    stroke: var(--accent-strong);
    stroke-width: 2.5;
  }
  ...
  .bs-ap.opposing .bs-ap-handle {
    stroke: var(--diag-dim);
    stroke-dasharray: 2 1.5;
  }
  .bs-ap.lit .bs-ap-handle,
  .bs-ap.dragging .bs-ap-handle {
    fill: var(--accent);
    stroke: var(--accent-strong);
  }
  .bs-ap:focus-visible .bs-ap-handle {
    fill: var(--accent-soft);
    stroke: var(--accent-strong);
    stroke-width: 2.2;
  }
  ...
  .bs-lc.lit .bs-lc-rule {
    stroke-dasharray: none;
    stroke-width: 2.4;
  }
  .bs-lc:focus-visible .bs-lc-rule {
    stroke-width: 3;
    stroke-dasharray: none;
  }
  ...
  .bs-zone.lit .bs-zone-bar {
    stroke-width: 2.6;
  }
  .bs-zone:focus-visible .bs-zone-bar {
    stroke-width: 3;
  }
  ...
  .bs-grade.lit .bs-grade-bar {
    stroke: var(--accent);
    stroke-width: 2.6;
  }
  .bs-grade:focus-visible .bs-grade-bar {
    stroke: var(--accent-strong);
    stroke-width: 3;
  }
  ...
  .bs-pass.lit .bs-pass-handle,
  .bs-pass.dragging .bs-pass-handle {
    fill: var(--accent);
    stroke: var(--accent-strong);
  }
  .bs-pass:focus-visible .bs-pass-handle {
    fill: var(--accent-soft);
    stroke: var(--accent-strong);
    stroke-width: 2.2;
  }
  ...
  .bs-curve.lit .bs-curve-bow,
  .bs-curve.dragging .bs-curve-bow {
    stroke: var(--accent);
    stroke-width: 2.4;
  }
  .bs-curve.lit .bs-curve-handle,
  .bs-curve.dragging .bs-curve-handle {
    fill: var(--accent);
    stroke: var(--accent-strong);
  }
  .bs-curve:focus-visible .bs-curve-handle {
    fill: var(--accent-soft);
    stroke: var(--accent-strong);
    stroke-width: 2.2;
  }
  ...
  .bs-demand.lit .bs-demand-rule,
  .bs-demand.dragging .bs-demand-rule {
    stroke: var(--accent);
    stroke-dasharray: none;
  }
  .bs-demand.lit .bs-demand-handle,
  .bs-demand.dragging .bs-demand-handle {
    fill: var(--accent);
    stroke: var(--accent-strong);
  }
  .bs-demand:focus-visible .bs-demand-handle {
    fill: var(--accent-soft);
    stroke: var(--accent-strong);
    stroke-width: 2.2;
  }
