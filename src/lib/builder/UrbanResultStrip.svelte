...
<section class="us-wrap" aria-label="Segment results" data-testid="urban-result-strip">
  ...
  <p class="us-single" data-testid="urban-single-period">
    The Chapter 16 and 18 methods are single-period, so this is one value per segment rather than a time-space grid. The Chapter 17 handoff below is where variation over time is described, as a distribution rather than as periods.
  </p>
      ...
      <tbody>
        ...
        <tr bind:this={rowEl} onkeydown={onRowKey}>
          ...
          {#each result.segments as s, i}
            ...
            <td class="us-cell" class:selected={selected === i}
                data-testid="urban-cell" data-cell-index={i}
                data-measure={measure.id} data-value={v ?? ''}
                style="background:{style.fill}; color:{style.ink}"
                tabindex={focusIndex === i ? 0 : -1}
                role="button"
                aria-label="segment {i + 1}, {measure.label} {cellText(measure, v)}{measure.unit ? ` ${measure.unit}` : ''}"
  ...
  {#if detail}
    <div class="us-detail" data-testid="urban-cell-detail">
      ...
      <dl>
        <div><dt>Length</dt><dd>{ft(detail.lengthFt)} ft</dd></div>
        <div><dt>Through lanes</dt><dd>{detail.lanes}</dd></div>
        <div><dt>Boundary control</dt><dd>{detail.control ?? '–'}</dd></div>
        <div><dt>Travel speed</dt><dd>{n(detail.travelSpeed, 2)} mi/h</dd></div>
        <div><dt>Base free-flow speed</dt><dd>{n(detail.baseFfs, 2)} mi/h</dd></div>
        <div><dt>Spatial stop rate</dt><dd>{n(detail.spatialStopRate, 2)} stops/mi</dd></div>
        <div><dt>Through v/c</dt><dd>{n(detail.vcRatio, 3)}</dd></div>
        <div><dt>LOS</dt><dd>{detail.los ?? '–'}</dd></div>
        {#if result.mode === 'inputs'}
          <div><dt>Access-point delay</dt><dd>{SOURCE_LABEL[detail.apDelaySource] ?? '–'}</dd></div>
        {/if}
        {#if detail.overridden}
          <div><dt>Override</dt><dd>This row was pinned by hand, so it does not follow the signals.</dd></div>
        {/if}
      </dl>
    </div>
  {/if}
...
<style>
  .us-wrap { margin-top: 1.25rem; }
  .us-head { display: flex; align-items: baseline; justify-content: space-between; gap: 1rem; flex-wrap: wrap; }
  .us-head h2 { font-size: 1rem; margin: 0 0 0.35rem; }
  .us-measure { font-size: 0.75rem; color: var(--text-secondary); display: inline-flex; align-items: center; gap: 0.3rem; }
  .us-note, .us-single { font-size: 0.76rem; color: var(--text-muted); margin: 0 0 0.4rem; max-width: 88ch; line-height: 1.5; }

  .us-scroll { overflow-x: auto; }
  .us-grid { border-collapse: collapse; font-size: 0.8rem; }
  .us-caption { caption-side: bottom; font-size: 0.72rem; color: var(--text-faint); text-align: left; padding-top: 0.3rem; }
  .us-grid th, .us-grid td { border: 1px solid var(--border); padding: 0.25rem 0.5rem; }
  .us-col { font-size: 0.72rem; color: var(--text-muted); font-weight: 600; text-align: center; }
  .us-col-sub { display: block; font-weight: 400; font-size: 0.66rem; color: var(--text-faint); font-variant-numeric: tabular-nums; }
  ...
  .us-rowhead, .us-corner { position: sticky; left: 0; background: var(--surface); text-align: left; font-size: 0.74rem; color: var(--text-muted); font-weight: 600; white-space: nowrap; z-index: 1; }
  .us-cell { text-align: center; font-variant-numeric: tabular-nums; font-weight: 700; cursor: pointer; min-width: 4.2rem; }
  .us-cell.selected { outline: 2px solid var(--accent-strong); outline-offset: -2px; }
  .us-cell:focus-visible { outline: 2px solid var(--accent-strong); outline-offset: -2px; }

  .us-legend { display: flex; align-items: center; gap: 0.15rem; margin: 0.5rem 0 0; flex-wrap: wrap; }
  .us-stop { display: inline-flex; align-items: center; justify-content: center; width: 1.5rem; height: 1.1rem; font-size: 0.68rem; font-weight: 700; border-radius: 2px; }
  .us-stop.us-ramp { width: 1.9rem; }
  .us-legend-end { font-size: 0.68rem; color: var(--text-muted); font-variant-numeric: tabular-nums; margin: 0 0.25rem; }

  .us-detail { margin-top: 0.75rem; padding: 0.5rem 0.6rem; border: 1px solid var(--border); border-radius: 4px; background: var(--surface-subtle); }
  .us-detail h3 { font-size: 0.86rem; margin: 0 0 0.3rem; }
  .us-detail dl { display: flex; flex-wrap: wrap; gap: 0.3rem 1.4rem; margin: 0; }
  .us-detail dt { font-size: 0.68rem; color: var(--text-muted); }
  .us-detail dd { margin: 0; font-size: 0.8rem; font-variant-numeric: tabular-nums; }
</style>
