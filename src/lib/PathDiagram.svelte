...
<div class="path-diagram">
  ...
  <p class="pd-note">
    ...
    {#if kind === 'bicycle'}Mode shares and speeds are the Exhibit 24-6 defaults. After a run, the share of bicycles stuck at walking pace follows the LOS, which is the delayed-passing effect the BLOS score measures.{/if}
    {#if kind === 'shared_path'}The highlighted walker is the average pedestrian the method scores: bicycles overtaking it are passing events, bicycles toward it are meetings.{/if}
    {#if kind === 'pedestrian'}Pedestrian space comes from the effective width after fixed objects and the platooned or random flow pattern.{/if}
...
<style>
  ...
  .pd-pavement { fill: var(--diag-pavement); }
  .pd-object { fill: var(--diag-pavement-alt); }
  .pd-edge { stroke: var(--diag-edge); stroke-width: 2; vector-effect: non-scaling-stroke; }
  .pd-centerline { stroke: var(--diag-center); stroke-width: 1.5; stroke-dasharray: 10 7; vector-effect: non-scaling-stroke; }
  .pd-centerline-soft { stroke: var(--diag-lane-line); stroke-width: 1.25; stroke-dasharray: 4 8; vector-effect: non-scaling-stroke; opacity: 0.6; }
  .pd-label { font-size: 9px; fill: var(--diag-dim); }

  .pd-user { transition: opacity 120ms ease; opacity: 0.9; }
  .pd-user.dim { opacity: 0.1; }
  .pd-user.active { opacity: 1; }
  .pd-user rect, .pd-user circle { stroke: rgba(15, 23, 42, 0.35); stroke-width: 0.6; }
  .u-bike rect { fill: #2563eb; }
  .u-bike.delayed rect { fill: #93b6f8; }
  .u-ped circle { fill: #16a34a; }
  .pd-user.probe circle { fill: #16a34a; stroke: #0f172a; stroke-width: 1.6; r: 4; }
  .u-run circle { fill: #ea7317; }
  .u-skate rect { fill: #8b5cf6; }
  .u-child rect { fill: #dc2626; }
  .swatch.u-bike { background: #2563eb; }
  .swatch.u-ped { background: #16a34a; }
  .swatch.u-run { background: #ea7317; }
  .swatch.u-skate { background: #8b5cf6; }
  .swatch.u-child { background: #dc2626; }

  .pd-legend { display: flex; flex-wrap: wrap; gap: 0.4rem; margin-top: 0.5rem; }
  .pd-chip {
    display: inline-flex;
    align-items: center;
    gap: 0.35rem;
  ...
  .pd-chip.active { border-color: var(--diag-edge); }
  .pd-animate { cursor: pointer; font-weight: 600; }
  .swatch { width: 0.7rem; height: 0.7rem; border-radius: 2px; display: inline-block; }
  .pd-note { font-size: 0.72rem; color: var(--text-muted); margin-top: 0.35rem; }
  ...
  .pd-cluster-title { font-size: 7px; font-weight: 600; flex: none; }
  .pd-cluster input {
    box-sizing: border-box;
    width: 100%;
    min-width: 0;
  ...
  .pd-cluster input::-webkit-inner-spin-button { -webkit-appearance: none; margin: 0; }
  .pd-cluster input[type='number'] { -moz-appearance: textfield; appearance: textfield; }
</style>
