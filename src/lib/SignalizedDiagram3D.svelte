<script>
  ...
  function build(lNB, lSB, lEB, lWB, tNB, tSB, tEB, tWB) {
    ...
    const centers = [
      [[0, YS], [0, -wEB]], [[0, wWB], [0, YN]],
      [[XW, 0], [-wSB, 0]], [[wNB, 0], [XE, 0]],
    ];
    const laneLines = [];
    for (let i = 1; i < lNB; i++) laneLines.push([[i, YS], [i, -wEB]], [[i, wWB], [i, YN]]);
    for (let i = 1; i < lSB; i++) laneLines.push([[-i, YS], [-i, -wEB]], [[-i, wWB], [-i, YN]]);
    for (let i = 1; i < lEB; i++) laneLines.push([[XW, -i], [-wSB, -i]], [[wNB, -i], [XE, -i]]);
    for (let i = 1; i < lWB; i++) laneLines.push([[XW, i], [-wSB, i]], [[wNB, i], [XE, i]]);
    const stops = [
      [[0, -wEB - 0.12], [wNB, -wEB - 0.12]],
      [[-wSB, wWB + 0.12], [0, wWB + 0.12]],
      [[-wSB - 0.12, 0], [-wSB - 0.12, -wEB]],
      [[wNB + 0.12, wWB], [wNB + 0.12, 0]],
    ];
    ...
    const moves = {
      NB: {
        thru: [[xNB(tNB), YS], [xNB(tNB), YN]],
        left: [[xNB(0), YS], [xNB(0), -wEB], ...qSample([xNB(0), -wEB], [xNB(0), LANE / 2], [-wSB, LANE / 2]), [XW, LANE / 2]],
        right: [[xNB(lNB - 1), YS], [xNB(lNB - 1), -wEB], ...qSample([xNB(lNB - 1), -wEB], [xNB(lNB - 1), -wEB + LANE / 2], [wNB, -wEB + LANE / 2]), [XE, -wEB + LANE / 2]],
      },
      SB: {
        thru: [[xSB(tSB), YN], [xSB(tSB), YS]],
        left: [[xSB(0), YN], [xSB(0), wWB], ...qSample([xSB(0), wWB], [xSB(0), -LANE / 2], [wNB, -LANE / 2]), [XE, -LANE / 2]],
        right: [[xSB(lSB - 1), YN], [xSB(lSB - 1), wWB], ...qSample([xSB(lSB - 1), wWB], [xSB(lSB - 1), wWB - LANE / 2], [-wSB, wWB - LANE / 2]), [XW, wWB - LANE / 2]],
      },
      EB: {
        thru: [[XW, yEB(tEB)], [XE, yEB(tEB)]],
        left: [[XW, yEB(0)], [-wSB, yEB(0)], ...qSample([-wSB, yEB(0)], [LANE / 2, yEB(0)], [LANE / 2, wWB]), [LANE / 2, YN]],
        right: [[XW, yEB(lEB - 1)], [-wSB, yEB(lEB - 1)], ...qSample([-wSB, yEB(lEB - 1)], [-wSB + LANE / 2, yEB(lEB - 1)], [-wSB + LANE / 2, -wEB]), [-wSB + LANE / 2, YS]],
      },
      WB: {
        thru: [[XE, yWB(tWB)], [XW, yWB(tWB)]],
        left: [[XE, yWB(0)], [wNB, yWB(0)], ...qSample([wNB, yWB(0)], [-LANE / 2, yWB(0)], [-LANE / 2, -wEB]), [-LANE / 2, YS]],
        right: [[XE, yWB(lWB - 1)], [wNB, yWB(lWB - 1)], ...qSample([wNB, yWB(lWB - 1)], [wNB - LANE / 2, yWB(lWB - 1)], [wNB - LANE / 2, wWB]), [wNB - LANE / 2, YN]],
      },
    };
...
<style>
  ...
  .sd3-legend { display: flex; flex-wrap: wrap; gap: 0.4rem; margin-top: 0.35rem; }
  .sd3-chip {
    display: inline-flex;
    align-items: center;
    gap: 0.35rem;
