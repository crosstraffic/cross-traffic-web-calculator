<script>
  ...
  let aadtUsed = $derived(
    p.aadt_mode === 'direct'
      ? Number(p.aadt_veh)
      : Number(p.k_factor) > 0 ? Number(p.peak_hour_volume_veh_h) / Number(p.k_factor) : 0
  );
    ...
    try {
      ...
      setReport({
        ...
        inputs: [
          ...
          { label: 'Pedestrian platooning', value: config.pedestrian_platooning ? `yes, ${config.crosswalk_width_ft} ft crosswalk at ${config.pedestrian_flow_p_h} p/h` : 'no' },
          { label: 'AADT of the crossed street', value: p.aadt_mode === 'direct' ? `${fmt(aadtUsed, 0)} veh/day (entered)` : `${fmt(aadtUsed, 0)} veh/day (from ${config.peak_hour_volume_veh_h} veh/h at K = ${config.k_factor})` },
          { label: 'Countermeasures', value: [config.has_marked_crosswalk ? 'marked crosswalk' : null, config.has_median_refuge ? 'median refuge' : null, config.has_rrfb ? 'RRFB' : null].filter(Boolean).join(', ') || 'none' },
        ],
        resultTable: {
          columns: ['Stage', 't_c (s)', 't_c,G (s)', 'P_b', 'P_d', 'd_g (s)', 'd_gd (s)', 'h (s)', 'n', 'P(Y_1)', 'Delay (s)'],
          rows: r.stages.map((s, i) => [
            String(i + 1), fmt(s.critical_headway), fmt(s.group_critical_headway),
            fmt(s.prob_blocked_lane, 3), fmt(s.prob_delayed_crossing, 3),
            fmt(s.gap_delay), fmt(s.gap_delay_when_delayed), fmt(s.average_short_headway),
            String(s.yield_events), fmt(s.prob_yield[1] ?? 0, 3), fmt(s.delay),
          ]),
        },
        summary: [
          ...
          { label: 'O(S/D) no delay / delay (Equation 20-95)', value: `${fmt(r.odds_satisfied_no_delay, 2)} / ${fmt(r.odds_satisfied_delay, 2)}` },
          { label: 'P(D) no delay / delay (Equation 20-97)', value: `${fmt(r.prob_dissatisfied_no_delay * 100)} % / ${fmt(r.prob_dissatisfied_delay * 100)} %` },
        ...
        diagram: { kind: 'ped-crossing', props: { stages: config.stages, walkSpeed: config.walk_speed_fps, los: r.los, totalDelay: r.delay } },
      });
    } catch (err) {
...
<form id="hcm20ped" inert={!ready} onsubmit={(e) => { e.preventDefault(); runAnalysis(); }}>
  <section class="panel">
    <div class="panel-head">
      <div>
        <h2 class="panel-title">Scenario</h2>
        <p class="panel-sub">The three scenarios of HCM Chapter 32, TWSC Example Problem 2, which evaluate the same four-lane crossing under progressively stronger countermeasures. Selecting one loads its published inputs; every field below stays editable.</p>
      </div>
    </div>
    <div class="param-grid">
      <div class="param-field">
        <label for="PEDSCEN_input">Example Problem 2 Scenario</label>
        <select id="PEDSCEN_input" class="select select-bordered select-sm" value={scenario} onchange={(e) => applyScenario(e.target.value)}>
  ...
  <section class="panel">
    <div class="panel-head">
      <div>
        ...
        <p class="panel-sub">Step 1 of the method. A crossing without a median refuge is one stage spanning the street. A refuge splits it into one stage per side, each carrying only the lanes and the conflicting flow of its own side, so a 46-ft crossing of four lanes at 1,700 veh/h becomes two 20-ft crossings of two lanes at 850 veh/h.</p>
      </div>
    </div>
      ...
      <div class="param-grid stage-row" data-testid={`ped-stage-row-${i + 1}`}>
        <div class="param-field">
          ...
          <div class="cell-field">
            <input id={`PEDL${i}_input`} type="number" step="0.1" min="0" class="input input-bordered input-sm" bind:value={stage.crossing_length_ft} required />
        ...
        <div class="param-field">
          ...
          <div class="cell-field">
            <input id={`PEDN${i}_input`} type="number" step="1" min="1" max="6" class="input input-bordered input-sm" bind:value={stage.through_lanes} required />
          </div>
        </div>
        <div class="param-field">
          <label for={`PEDV${i}_input`}>Stage {i + 1} Conflicting Flow</label>
          <div class="cell-field">
            <input id={`PEDV${i}_input`} type="number" step="1" min="0" class="input input-bordered input-sm" bind:value={stage.conflicting_flow_veh_h} required />
            <span class="unit">veh/h</span>
          </div>
          <p class="param-hint">Both directions for a one-stage crossing, the crossed side only for a stage of a two-stage crossing.</p>
        </div>
      </div>
      <div class="stage-action">
        <button type="button" class="btn btn-ghost btn-sm" onclick={() => removeStage(i)} disabled={p.stages.length <= 1}>Remove stage {i + 1}</button>
      </div>
    {/each}
    ...
    <div class="param-grid">
      <div class="param-field">
        ...
        <p class="param-hint">The HCM works one- and two-stage crossings. More stages are summed by Equation 20-94 the same way but have no published example behind them.</p>
      </div>
    </div>

    <div class="diagram-block">
      <div class="diagram-head">
        <p class="panel-sub">Cross-section along the pedestrian's line of travel, showing the lanes crossed in each stage and the refuge between them. The conflicting stream is a flow rate in this method rather than located vehicles, so it is shown as a direction and not as traffic.</p>
      </div>
      <PedCrossingDiagram
  ...
  <section class="panel">
    <div class="panel-head">
      <div>
        ...
        <p class="panel-sub">Walking speed and start-up time set the critical headway (Equation 20-76). The platoon adjustment of Equations 20-77 through 20-79 applies only where pedestrians are observed to group.</p>
      </div>
    </div>
    <div class="param-grid">
      <div class="param-field">
        ...
        <div class="cell-field">
          <input id="PEDSP_input" type="number" step="0.1" min="0.5" max="10" class="input input-bordered input-sm" bind:value={p.walk_speed_fps} required />
          <span class="unit">ft/s</span>
        </div>
        <p class="param-hint">3.5 ft/s is the 15th-percentile design value under Exhibit 20-26. The measured average at uncontrolled crossings is 4.7 ft/s, and Example Problem 2 uses 4.0.</p>
      </div>

      <div class="param-field">
        <label for="PEDTS_input">Start-Up and End Clearance</label>
        <div class="cell-field">
          <input id="PEDTS_input" type="number" step="0.1" min="0" max="20" class="input input-bordered input-sm" bind:value={p.startup_clearance_s} required />
      ...
      <div class="param-field">
        ...
        <p class="param-hint">With no platooning the spatial distribution is one row and Equations 20-77 and 20-78 are skipped.</p>
      </div>
      ...
      {#if p.pedestrian_platooning}
        <div class="param-field">
          ...
          <div class="cell-field">
            <input id="PEDWC_input" type="number" step="0.5" min="1" class="input input-bordered input-sm" bind:value={p.crosswalk_width_ft} required />
            <span class="unit">ft</span>
          </div>
        </div>
        ...
        <div class="param-field">
          ...
          <div class="cell-field">
            <input id="PEDVP_input" type="number" step="1" min="0" class="input input-bordered input-sm" bind:value={p.pedestrian_flow_p_h} required />
  ...
  <section class="panel">
    <div class="panel-head">
      <div>
        ...
        <p class="panel-sub">The yield rate drives Step 5, and the three indicators together with the AADT drive the Step 7 satisfaction model (Equation 20-95).</p>
      </div>
    </div>
    <div class="param-grid">
      <div class="param-field">
        ...
        <div class="cell-field">
          <input id="PEDMY_input" type="number" step="1" min="0" max="100" class="input input-bordered input-sm" bind:value={p.yield_pct} required />
          <span class="unit">%</span>
        </div>
        <p class="param-hint">Exhibit 20-28 gives observed rates by crossing treatment. A 100% rate is entered as 99.99% by the engine, because the yield recursion is undefined at exactly 100%.</p>
      </div>
      ...
      <div class="param-field">
        ...
        <p class="param-hint">This is the Equation 20-95 indicator. It is a separate input from the staging above, which is where the refuge changes the delay calculation.</p>
      </div>
      ...
      {#if p.aadt_mode === 'direct'}
        <div class="param-field">
          ...
          <div class="cell-field">
            <input id="PEDAADT_input" type="number" step="100" min="0" class="input input-bordered input-sm" bind:value={p.aadt_veh} required />
      ...
      {:else}
        <div class="param-field">
          ...
          <div class="cell-field">
            <input id="PEDPHV_input" type="number" step="1" min="0" class="input input-bordered input-sm" bind:value={p.peak_hour_volume_veh_h} required />
            <span class="unit">veh/h</span>
          </div>
        </div>
        ...
        <div class="param-field">
          ...
          <div class="cell-field">
            <input id="PEDK_input" type="number" step="0.01" min="0.01" max="1" class="input input-bordered input-sm" bind:value={p.k_factor} required />
          </div>
  ...
  <div class="action-bar">
    <button class="btn btn-ghost" onclick={() => applyScenario(scenario)} type="button">Reset to Scenario {scenario}</button>
  ...
  <div class="los overflow-x-auto">
    ...
    {#if results}
      <div class="yield-chain">
        ...
        <p class="panel-sub">Probability that every blocking vehicle yields on potential yielding event i. P(Y_0) is zero by definition, because there is nothing to yield to before the first event.</p>
        <table class="table w-full">
          <tbody>
            {#each results.stages as s, i}
    ...
    <table class="table w-full">
      <tbody>
        ...
        <tr>
          ...
          <td>{results ? `${fmt(results.odds_satisfied_no_delay, 2)} / ${fmt(results.odds_satisfied_delay, 2)}` : ''}</td>
        </tr>
        <tr>
          <th>P(Dissatisfied), No Delay / Delay (Equation 20-97):</th>
          <td>{results ? `${fmt(results.prob_dissatisfied_no_delay * 100)} % / ${fmt(results.prob_dissatisfied_delay * 100)} %` : ''}</td>
        </tr>
        <tr>
  ...
  {#if results}
    <div class="facility-summary">
        <div class="ped-los" data-testid="ped-los">
          ...
          <p>
            Pedestrian LOS comes from the average proportion of pedestrians who would rate the
            crossing "dissatisfied" or worse (Exhibit 20-3), not from the delay. The bands are
            A below 0.05, B below 0.15, C below 0.25, D below 0.33, E below 0.50, and F at or
            above 0.50. This crossing sits at {fmt(results.proportion_dissatisfied, 3)}.
          </p>
        </div>
        <p>
          The Equation 20-95 coefficients on the median-refuge and non-yielding indicators are
          clipped out of the HCM 7th Edition text by its typesetting and do not appear in the
          Chapter 20 prose. The engine recovers them by fitting the six published O(S/D) values
          of this example problem, which over-determine the two unknowns, so the satisfaction
          rows above carry that provenance rather than a published coefficient.
        </p>
        {#if results.stageCount > 1}
          <p>
            On a two-stage crossing the HCM does not say which stage's P_d and P(Y_1) feed
            Equations 20-98 and 20-99. The engine uses the first stage, which reproduces the
            published P_nd of 0.481 for Scenario B. Example Problem 2 cannot settle the question,
            because its two stages are identical by construction.
          </p>
        {/if}
    </div>
  {/if}
...
<style>
  .stage-row { align-items: start; }
  .stage-action { display: flex; justify-content: flex-end; margin: -0.25rem 0 0.75rem; }
  .subhead { font-size: 0.95rem; font-weight: 600; margin: 1rem 0 0.25rem; }
  .ped-los { display: flex; gap: 1rem; align-items: flex-start; margin-bottom: 0.75rem; }
  .ped-los p { margin: 0; }
</style>
