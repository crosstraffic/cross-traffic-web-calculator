<script>
  ...
  /** @type {Props} */
  let { laneCount = 3, laneWidth = 12, length = 0.625, grade = 0, lcR = 6 } = $props();
  ...
  const DEF_YAW = 35,
    DEF_PITCH = 30;
  let yaw = $state(DEF_YAW),
    pitch = $state(DEF_PITCH);
  let zoom = $state(1),
    panX = $state(0),
    panY = $state(0);
  ...
  function build(N, lw, len, grade, lcR, yaw, pitch, zoom, panX, panY) {
    ...
    const dividersW = [];
    for (let d = 1; d < N; d++) {
      const x = halfW - d * laneUnit;
      dividersW.push(st.map((_, i) => wp(i, x)));
    }
    ...
    const poly = (a, b, dy = 0) =>
      'M' +
      a.map((p) => `${p.x.toFixed(1)},${(p.y + dy).toFixed(1)}`).join(' L') +
      ' L' +
      [...b]
        .reverse()
        .map((p) => `${p.x.toFixed(1)},${(p.y + dy).toFixed(1)}`)
        .join(' L') +
      ' Z';
    const wall = (edge) =>
      'M' +
      edge.map((p) => `${p.x.toFixed(1)},${p.y.toFixed(1)}`).join(' L') +
      ' L' +
      [...edge]
        .reverse()
        .map((p) => `${p.x.toFixed(1)},${(p.y + THICK).toFixed(1)}`)
        .join(' L') +
      ' Z';
    const endFace = (m, n) =>
    ...
    const cMidA = {
      x: (Lf[Lf.length - 2].x + Rf[Rf.length - 2].x) / 2,
      y: (Lf[Lf.length - 2].y + Rf[Rf.length - 2].y) / 2,
    };
    const cMidB = {
      x: (Lf[Lf.length - 1].x + Rf[Rf.length - 1].x) / 2,
      y: (Lf[Lf.length - 1].y + Rf[Rf.length - 1].y) / 2,
    };
