...
export function fitTransform(project, planPts, viewW, viewH, pad, zoom, panX, panY, thick, fill = 1.28) {
  const xs = planPts.map((p) => p[0]),
    ys = planPts.map((p) => p[1]),
    zs = planPts.map((p) => p[2] || 0);
...
export function makeDrawers(tf, thick) {
  ...
  const shadow = (pts, drop = 3) =>
    ...
    pts
      .map((p) => {
        const q = pt(p);
        return `${q.x.toFixed(1)},${(q.y + thick + drop).toFixed(1)}`;
      })
      .join(' L') +
    ' Z';
  ...
  const walls = (pts) => {
    ...
    for (let i = 0; i < pts.length; i++) {
      const a = pt(pts[i]),
        b = pt(pts[(i + 1) % pts.length]);
      out.push(
        `M${fmt(a)} L${fmt(b)} L${b.x.toFixed(1)},${(b.y + thick).toFixed(1)} L${a.x.toFixed(1)},${(a.y + thick).toFixed(1)} Z`,
      );
    }
    return out;
  };
