<script>
   ...
   * @property {boolean} [editable] - the printable report.
   * @property {number} [cycleLength] - LOS letters from the last run; congested approaches discharge slower.
   * @property {any} [approachLos]
   */
  ...
  let wNB = $derived(lanes('NB') * LANE);
  let wSB = $derived(lanes('SB') * LANE);
  let wEB = $derived(lanes('EB') * LANE);
  let wWB = $derived(lanes('WB') * LANE);
  ...
  let iLeft = $derived(() => 0);
  let iThru = $derived((key) => nL(key) + Math.floor((nT(key) - 1) / 2));
  let iRight = $derived((key) => lanes(key) - 1);
