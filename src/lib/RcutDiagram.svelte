<script>
  ...
  const stemN = jy - STEM, stemS = jy + STEM;
  ...
  const sbThru = cx - 30, sbRight = cx - 23, sbMedian = cx - 14;
  const nbLeft = cx + 13, nbThru = cx + 23, nbReturn = cx + 30;
  const stemL = jy + 4, stemR = jy + 10;               // eastbound stem lane
  const outN = jy - 10, outS = jy - 4;                 // westbound stem lane
  ...
  let openN = $derived(xy - 16), openS = $derived(xy + 16);   // crossover median opening
  let loonN = $derived(xy - 26), loonS = $derived(xy + 26);
  ...
  let loonFill = $derived(`M ${nbE},${loonN} L ${cx + 37},${loonN} A 26 26 0 0 1 ${cx + 37},${loonS} L ${nbE},${loonS} Z`);
  let loonEdge = $derived(`M ${nbE},${loonN} L ${cx + 37},${loonN} A 26 26 0 0 1 ${cx + 37},${loonS} L ${nbE},${loonS}`);
  ...
  let P = $derived({
    ...
    ebl: `M 0,${stemL} L ${cx - 36},${stemL} Q ${sbMedian},${stemL} ${sbMedian},${jy + 26} L ${sbMedian},${xy - 26}`
  ...
  const BASE_COLOR = {
    ebl: '#2563eb', ebr: '#0891b2', nbl: '#16a34a',
    nbt: '#65a30d', sbt: '#ea7317', sbr: '#dc2626'
  };
...
<div class="rc-diagram">
  ...
  <div class="rc-legend">
    <button type="button" class="rc-chip rc-animate" class:active={animating}
  ...
  <p class="rc-note">The minor-street left turn is served by the crossover rather than at the main junction, so it meets two STOP-controlled junctions and carries the extra distance travel time. Paths sharing a lane are drawn slightly apart to stay readable, and the crossover position tracks the dimensioned distance. Animated traffic slows per movement LOS after a run. An illustration, not a simulation.</p>
</div>
...
<style>
  ...
  .rc-pavement { fill: var(--diag-pavement); }
  .rc-island { fill: var(--diag-wall); }
  .rc-island-edge { stroke: var(--diag-wall-edge); stroke-width: 1; vector-effect: non-scaling-stroke; }
  .rc-edge { stroke: var(--diag-edge); stroke-width: 1.5; stroke-linecap: round; vector-effect: non-scaling-stroke; }
  .rc-center { stroke: var(--diag-center); stroke-width: 1.5; vector-effect: non-scaling-stroke; }
  .rc-lane-line { stroke: var(--diag-lane-line); stroke-width: 1.25; stroke-dasharray: 8 6; vector-effect: non-scaling-stroke; }
  .rc-stop { stroke: var(--diag-lane-line); stroke-width: 3.5; vector-effect: non-scaling-stroke; }
  .rc-dim { stroke: var(--diag-dim); stroke-width: 1; vector-effect: non-scaling-stroke; }
  .rc-ext { stroke: var(--diag-dim); stroke-width: 0.8; stroke-dasharray: 3 3; vector-effect: non-scaling-stroke; }
  .rc-compass polygon { fill: var(--diag-dim); }
  ...
  .rc-move.dim { opacity: 0.08; }
  .rc-move.active { stroke-width: 4; opacity: 1; }

  .rc-veh rect { stroke: rgba(15, 23, 42, 0.35); stroke-width: 0.6; }
  .rc-veh { transition: opacity 120ms ease; }
  .rc-veh.dim { opacity: 0.08; }

  .rc-label { font-size: 9px; fill: var(--text-muted); }
  .rc-label.mid { text-anchor: middle; }
  .rc-label.end { text-anchor: end; }

  .rc-legend { display: flex; flex-wrap: wrap; gap: 0.4rem; margin-top: 0.5rem; }
  .rc-chip {
    display: inline-flex;
    align-items: center;
    gap: 0.35rem;
  ...
  .rc-chip.active { border-color: var(--diag-edge); }
  .rc-animate { cursor: pointer; font-weight: 600; }
  .swatch { width: 0.7rem; height: 0.7rem; border-radius: 2px; display: inline-block; }
  .rc-note { font-size: 0.72rem; color: var(--text-muted); margin-top: 0.35rem; }
  ...
  .rc-cluster-title { font-size: 7px; font-weight: 600; flex: none; }
  .rc-cluster input {
    box-sizing: border-box;
    width: 100%;
    min-width: 0;
  ...
  .rc-cluster input::-webkit-inner-spin-button { -webkit-appearance: none; margin: 0; }
  .rc-cluster input[type='number'] { -moz-appearance: textfield; appearance: textfield; }
</style>
