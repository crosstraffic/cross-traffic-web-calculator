<script>
  ...
  /** @type {Props} */
  let { weavingType = 'one_sided', numLanes = 4, vFF = 0, vFR = 0, vRF = 0, vRR = 0 } = $props();
  ...
  function build(twoSided, mainLanes) {
    ...
    if (twoSided) {
      const onr = [
        [gIn - RAMP, mainBot + DROP],
        [gIn, mainBot],
        [gIn + 70, mainBot],
        [gIn - RAMP, mainBot + DROP + LANE],
      ];
      const offr = [
        [gOut - 70, mainTop],
        [gOut + RAMP, mainTop - DROP - LANE],
        [gOut + RAMP, mainTop - DROP],
        [gOut, mainTop],
      ];
      slabs = [onr, offr, main];
      ...
      moves = {
        ff: [
          [0, yLane(Math.floor((mainLanes - 1) / 2))],
          [X1 - 8, yLane(Math.floor((mainLanes - 1) / 2))],
        ],
        rr: [
          [ri.x, ri.y],
          [gIn + 2, mainBot + 9],
        ...
        rf: [
          [ri.x, ri.y + 4],
          [gIn + 4, mainBot + 12],
          ...qSample([gIn + 4, mainBot + 12], [gIn + 56, mainBot], [gIn + 96, yBottom]),
          [X1 - 8, yBottom],
        ],
        fr: [
          [0, yTop],
          [gOut - 70, yTop],
          ...qSample([gOut - 70, yTop], [gOut - 16, yTop - 6], [gOut + 2, mainTop - 7]),
          [ro.x, ro.y + 4],
        ],
      };
    } else {
      ...
      const onr = [
        [gIn - RAMP, auxBot + DROP - LANE],
        [gIn, mainBot],
        [gIn, auxBot],
        [gIn - RAMP, auxBot + DROP],
      ];
      const offr = [
        [gOut, mainBot],
        [gOut + RAMP, auxBot + DROP - LANE],
        [gOut + RAMP, auxBot + DROP],
        [gOut, auxBot],
      ];
      ...
      moves = {
        ...
        rf: [
          [ri.x, ri.y - 3],
          [gIn, yAux - 3],
          ...cSample([gIn, yAux - 3], [gIn + 52, yAux - 2], [gIn + 84, yBottom], [gIn + 128, yBottom]),
          [X1 - 8, yBottom],
        ],
        fr: [
          [0, yBottom],
          [gIn + 24, yBottom],
          ...cSample([gIn + 24, yBottom], [gIn + 60, yBottom], [gIn + 72, yAux - 2], [gIn + 108, yAux - 2]),
          [gOut, yAux - 2],
          [ro.x, ro.y - 3],
        ],
        rr: [
