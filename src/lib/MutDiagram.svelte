<script>
  ...
  const cx = 240, jy = 220;             // main-street centreline, main junction
  ...
  const eT = jy + 15, eL = jy + 22, eR = jy + 28;   // eastbound through / left / right
  const wT = jy - 15, wL = jy - 22, wR = jy - 28;   // westbound through / left / right
  ...
  let P = $derived({
    ...
    nbl: `M ${NB_L},${H} L ${NB_L},${xyN + 14} C ${NB_L},${xyN - 14} ${SB_N},${xyN - 14} ${SB_N},${xyN + 14}`
      + ` L ${SB_N},${jy - 44} Q ${SB_N},${wR} ${sbW - 18},${wR} L 0,${wR}`,
    wbl: `M ${W},${wL} L ${nbE + 18},${wL} Q ${NB_S},${wL} ${NB_S},${jy - 44} L ${NB_S},${xyN + 14}`
      + ` C ${NB_S},${xyN - 20} ${SB_W},${xyN - 20} ${SB_W},${xyN + 14} L ${SB_W},${H}`,
    // South-crossover U-turns.
    sbl: `M ${SB_L},0 L ${SB_L},${xyS - 14} C ${SB_L},${xyS + 14} ${NB_S},${xyS + 14} ${NB_S},${xyS - 14}`
      + ` L ${NB_S},${jy + 44} Q ${NB_S},${eL} ${nbE + 18},${eL} L ${W},${eL}`,
    ebl: `M 0,${eL} L ${sbW - 18},${eL} Q ${SB_N},${eL} ${SB_N},${jy + 44} L ${SB_N},${xyS - 14}`
...
<div class="mu-diagram">
  <svg viewBox="0 0 {W} {H}" preserveAspectRatio="xMidYMid meet" role="img" aria-label={ariaLabel}>
    ...
    {#each editable ? GROUPS : [] as g (g.key)}
      <foreignObject x={clusterPos[g.key].x} y={clusterPos[g.key].y} width={CW} height={CH}>
        <div class="mu-cluster" xmlns="http://www.w3.org/1999/xhtml"
  ...
  <div class="mu-legend">
    <button type="button" class="mu-chip mu-animate" class:active={animating}
...
<style>
  ...
  .mu-pavement { fill: var(--diag-pavement); }
  .mu-island { fill: var(--diag-wall); }
  .mu-island-edge { stroke: var(--diag-wall-edge); stroke-width: 1; vector-effect: non-scaling-stroke; }
  .mu-edge { stroke: var(--diag-edge); stroke-width: 1.5; stroke-linecap: round; vector-effect: non-scaling-stroke; }
  .mu-center { stroke: var(--diag-center); stroke-width: 1.25; vector-effect: non-scaling-stroke; }
  .mu-lane-line { stroke: var(--diag-lane-line); stroke-width: 1.25; stroke-dasharray: 8 6; vector-effect: non-scaling-stroke; }
  .mu-stop { stroke: var(--diag-lane-line); stroke-width: 3.5; vector-effect: non-scaling-stroke; }
  .mu-signal { fill: var(--diag-center); stroke: var(--diag-edge); stroke-width: 1.25; }
  .mu-dim { stroke: var(--diag-dim); stroke-width: 1; vector-effect: non-scaling-stroke; }
  .mu-ext { stroke: var(--diag-dim); stroke-width: 0.8; stroke-dasharray: 3 3; vector-effect: non-scaling-stroke; }
  .mu-compass polygon { fill: var(--diag-dim); }
  ...
  .mu-move.dim { opacity: 0.07; }
  .mu-move.active { stroke-width: 3.5; opacity: 1; }

  .mu-veh rect { stroke: rgba(15, 23, 42, 0.35); stroke-width: 0.6; }
  .mu-veh { transition: opacity 120ms ease; }
  .mu-veh.dim { opacity: 0.07; }

  .mu-label { font-size: 9px; fill: var(--text-muted); }
  .mu-label.mid { text-anchor: middle; }
  .mu-label.end { text-anchor: end; }

  .mu-legend { display: flex; flex-wrap: wrap; gap: 0.4rem; margin-top: 0.5rem; }
  .mu-chip {
    display: inline-flex;
    align-items: center;
    gap: 0.35rem;
  ...
  .mu-chip.active { border-color: var(--diag-edge); }
  .mu-animate { cursor: pointer; font-weight: 600; }
  ...
  .mu-note { font-size: 0.72rem; color: var(--text-muted); margin-top: 0.35rem; }
  ...
  .mu-cluster-title { display: inline-flex; align-items: center; gap: 2px; font-size: 7px; font-weight: 600; flex: none; }
  .mu-cluster input {
    box-sizing: border-box;
    width: 100%;
    min-width: 0;
  ...
  .mu-cluster input::-webkit-inner-spin-button { -webkit-appearance: none; margin: 0; }
  .mu-cluster input[type='number'] { -moz-appearance: textfield; appearance: textfield; }
  .mu-cluster .swatch { width: 6px; height: 6px; }
</style>
