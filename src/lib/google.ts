...
export function initializeGoogleAccounts() {
    ...
    async function googleCallback(response: googleInitialized.accounts.id.CredentialResponse) {
        ...
        if (res.ok) {
            ...
            if (role === 'base') return goto('/base')
            if (role === 'advanced') return goto('/advanced')
