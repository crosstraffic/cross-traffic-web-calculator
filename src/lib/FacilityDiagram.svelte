<script>
  ...
  function wzNote(seg) {
    ...
    return seg.wz.mismatch
      ? `${base}, but the segment is coded with ${seg.lanes} lanes, which is what the run uses`
      : base;
  }
  ...
  >
    ...
    {#each layout as seg, i}
      ...
      <g
        ...
        class:wz={!!seg.wz}
        data-wz-open={seg.wz ? seg.wz.open : null}
        data-wz-closed={seg.wz ? seg.wz.closed : null}
        onclick={() => pick(i)}
        role="button"
        tabindex="-1"
        aria-label="segment {seg.num}, {seg.type}{losFor(i) ? `, LOS ${losFor(i)}` : ''}{seg.ml
          ? ', managed lane'
          : ''}{seg.wz ? `, ${wzNote(seg)}` : ''}"
      >
        <title
          >Segment {seg.num} · {seg.type}{losFor(i)
            ? ` · LOS ${losFor(i)} · ${densityFor(i)?.toFixed(1)} veh/mi/ln`
            : ''}{seg.ml && mlLosFor(i)
            ? ` · ML LOS ${mlLosFor(i)} · ${mlDensityFor(i)?.toFixed(1)} veh/mi/ln`
            : ''}{seg.wz ? ` · ${wzNote(seg)}` : ''}</title
        >
        ...
        {#if seg.ml}
          <rect
            x={seg.x}
            y={top - ML_GAP - seg.ml * LANE}
            width={seg.w}
            height={seg.ml * LANE}
            fill={mlFillFor(i)}
            class="fd-ml"
            class:scored={mlLosFor(i) != null}
          />
          {#if mlLosFor(i)}
            <text
              x={seg.x + seg.w / 2}
              y={top - ML_GAP - (seg.ml * LANE) / 2 + 3.5}
              class="fd-ml-los"
              text-anchor="middle">{mlLosFor(i)}</text
            >
          {/if}
        {/if}
        ...
        {#if seg.type === 'Merge'}
          <polygon
            points="{seg.x - 14},{bot + RAMP_H} {seg.x - 4},{bot + RAMP_H} {seg.x + 10},{bot + RL} {seg.x},{bot + RL}"
            fill={fillFor(i)}
            class="fd-ramp"
            class:scored={losFor(i) != null}
          />
          <polygon
            points="{seg.x},{bot} {seg.x},{bot + RL} {seg.x + seg.w * 0.55},{bot + RL} {seg.x + seg.w * 0.78},{bot}"
            fill={fillFor(i)}
            class="fd-ramp"
            class:scored={losFor(i) != null}
          />
        ...
        {:else if seg.type === 'Diverge'}
          <polygon
            points="{seg.x + seg.w * 0.22},{bot} {seg.x + seg.w * 0.45},{bot + RL} {seg.x + seg.w},{bot + RL} {seg.x +
              seg.w},{bot}"
            fill={fillFor(i)}
            class="fd-ramp"
            class:scored={losFor(i) != null}
          />
          <polygon
            points="{seg.x + seg.w},{bot + RL} {seg.x + seg.w + 10},{bot + RL} {seg.x + seg.w + 14},{bot +
              RAMP_H} {seg.x + seg.w + 4},{bot + RAMP_H}"
            fill={fillFor(i)}
            class="fd-ramp"
            class:scored={losFor(i) != null}
          />
        ...
        {:else if seg.type === 'OverlappingRamp'}
          <polygon
            points="{seg.x - 12},{bot + RAMP_H} {seg.x - 2},{bot + RAMP_H} {seg.x + 8},{bot + RL} {seg.x},{bot + RL}"
            fill={fillFor(i)}
            class="fd-ramp"
            class:scored={losFor(i) != null}
          />
          <polygon
            points="{seg.x},{bot} {seg.x},{bot + RL} {seg.x + seg.w * 0.62},{bot + RL} {seg.x + seg.w * 0.62},{bot}"
            fill={fillFor(i)}
            class="fd-ramp"
            class:scored={losFor(i) != null}
          />
          <polygon
            points="{seg.x + seg.w * 0.38},{bot} {seg.x + seg.w * 0.38},{bot + RL} {seg.x + seg.w},{bot + RL} {seg.x +
              seg.w},{bot}"
            fill={fillFor(i)}
            class="fd-ramp"
            class:scored={losFor(i) != null}
          />
          <polygon
            points="{seg.x + seg.w},{bot + RL} {seg.x + seg.w + 8},{bot + RL} {seg.x + seg.w + 12},{bot +
              RAMP_H} {seg.x + seg.w + 2},{bot + RAMP_H}"
            fill={fillFor(i)}
            class="fd-ramp"
            class:scored={losFor(i) != null}
          />
        ...
        {#if losFor(i)}
          <text x={seg.x + seg.w / 2} y={top + (seg.open * LANE) / 2 + 3.5} class="fd-los" text-anchor="middle"
            >{losFor(i)}</text
          >
        {/if}
        {#if seg.wz}
          <text
            x={seg.x + seg.w / 2}
            y={seg.wz.closed > 0 ? trav + (seg.wz.closed * LANE) / 2 + 2.2 : trav - 2.5}
            class="fd-wz-chip"
            class:mismatch={seg.wz.mismatch}
            data-testid="wz-chip"
            text-anchor="middle">{seg.wz.label}</text
          >
        {/if}
        <text x={seg.x + seg.w / 2} y={H - 6} class="fd-type" text-anchor="middle"
          >{seg.type === 'OverlappingRamp' ? 'Ovlp' : seg.type}</text
        >
      </g>
    {/each}
    ...
    <!-- direction arrow -->
    <polygon
      points="{totalW - 26},{LABEL_TOP - 14} {totalW - 12},{LABEL_TOP - 10} {totalW - 26},{LABEL_TOP - 6}"
      class="fd-arrow"
    />
    ...
    {#if periods > 0}
      <div class="fd-periods" role="group" aria-label="Analysis period">
        {#each Array.from({ length: periods }) as _, p}
          <button type="button" class="fd-chip" class:active={period === p} onclick={() => (period = p)}
            >P{p + 1}</button
          >
        {/each}
      </div>
...
<style>
  ...
  .fd-seg.selected .fd-main,
  .fd-seg.selected .fd-ml {
    stroke: var(--accent);
    stroke-width: 2.5;
  }
  .fd-lane-line {
