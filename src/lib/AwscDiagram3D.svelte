<script>
  ...
  function build(nEB, nWB, nNB, nSB) {
    ...
    const hEB = Math.max(1, nEB) * LANE, hWB = Math.max(1, nWB) * LANE;
    const hNB = Math.max(1, nNB) * LANE, hSB = Math.max(1, nSB) * LANE;
    const XW = -(hSB + RUN), XE = hNB + RUN, YS = -(hEB + RUN), YN = hWB + RUN;
    ...
    const centers = [];
    if (legW) centers.push([[XW, 0], [-hSB, 0]]);
    if (legE) centers.push([[hNB, 0], [XE, 0]]);
    if (legS) centers.push([[0, YS], [0, -hEB]]);
    if (legN) centers.push([[0, hWB], [0, YN]]);

    const laneLines = [];
    for (let i = 1; i < nEB; i++) laneLines.push([[XW, -i], [-hSB, -i]]);
    for (let i = 1; i < nWB; i++) laneLines.push([[hNB, i], [XE, i]]);
    for (let i = 1; i < nNB; i++) laneLines.push([[i, YS], [i, -hEB]]);
    for (let i = 1; i < nSB; i++) laneLines.push([[-i, hWB], [-i, YN]]);

    const stops = [];
    if (legW) stops.push([[-hSB - 0.12, 0], [-hSB - 0.12, -hEB]]);
    if (legE) stops.push([[hNB + 0.12, hWB], [hNB + 0.12, 0]]);
    if (legS) stops.push([[0, -hEB - 0.12], [hNB, -hEB - 0.12]]);
    if (legN) stops.push([[-hSB, hWB + 0.12], [0, hWB + 0.12]]);
    ...
    const moves = {
      EB: !legW ? {} : {
        L: legN ? [[XW, yEB(0)], [-hSB, yEB(0)], ...qSample([-hSB, yEB(0)], [LANE / 2, yEB(0)], [LANE / 2, hWB]), [LANE / 2, YN]] : null,
        T: legE ? [[XW, yEB(mid(nEB))], [XE, yEB(mid(nEB))]] : null,
        R: legS ? [[XW, yEB(nEB - 1)], [-hSB, yEB(nEB - 1)], ...qSample([-hSB, yEB(nEB - 1)], [-hSB + LANE / 2, yEB(nEB - 1)], [-hSB + LANE / 2, -hEB]), [-hSB + LANE / 2, YS]] : null,
      },
      WB: !legE ? {} : {
        L: legS ? [[XE, yWB(0)], [hNB, yWB(0)], ...qSample([hNB, yWB(0)], [-LANE / 2, yWB(0)], [-LANE / 2, -hEB]), [-LANE / 2, YS]] : null,
        T: legW ? [[XE, yWB(mid(nWB))], [XW, yWB(mid(nWB))]] : null,
        R: legN ? [[XE, yWB(nWB - 1)], [hNB, yWB(nWB - 1)], ...qSample([hNB, yWB(nWB - 1)], [hNB - LANE / 2, yWB(nWB - 1)], [hNB - LANE / 2, hWB]), [hNB - LANE / 2, YN]] : null,
      },
      NB: !legS ? {} : {
        L: legW ? [[xNB(0), YS], [xNB(0), -hEB], ...qSample([xNB(0), -hEB], [xNB(0), LANE / 2], [-hSB, LANE / 2]), [XW, LANE / 2]] : null,
        T: legN ? [[xNB(mid(nNB)), YS], [xNB(mid(nNB)), YN]] : null,
        R: legE ? [[xNB(nNB - 1), YS], [xNB(nNB - 1), -hEB], ...qSample([xNB(nNB - 1), -hEB], [xNB(nNB - 1), -hEB + LANE / 2], [hNB, -hEB + LANE / 2]), [XE, -hEB + LANE / 2]] : null,
      },
      SB: !legN ? {} : {
        L: legE ? [[xSB(0), YN], [xSB(0), hWB], ...qSample([xSB(0), hWB], [xSB(0), -LANE / 2], [hNB, -LANE / 2]), [XE, -LANE / 2]] : null,
        T: legS ? [[xSB(mid(nSB)), YN], [xSB(mid(nSB)), YS]] : null,
        R: legW ? [[xSB(nSB - 1), YN], [xSB(nSB - 1), hWB], ...qSample([xSB(nSB - 1), hWB], [xSB(nSB - 1), hWB - LANE / 2], [-hSB, hWB - LANE / 2]), [XW, hWB - LANE / 2]] : null,
      },
    };
...
<style>
  .aw3-shadow { fill: #0f172a; opacity: 0.08; }
  .aw3-wall { fill: var(--diag-wall); stroke: var(--diag-wall-edge); stroke-width: 0.5; }
  .aw3-top { fill: var(--diag-pavement); stroke: var(--diag-edge); stroke-width: 1.5; stroke-linejoin: round; vector-effect: non-scaling-stroke; }
  .aw3-center { stroke: var(--diag-center); stroke-width: 1.25; fill: none; vector-effect: non-scaling-stroke; }
  .aw3-lane-line { stroke: var(--diag-lane-line); stroke-width: 1.25; stroke-dasharray: 7 5; fill: none; vector-effect: non-scaling-stroke; }
  .aw3-stop { stroke: var(--diag-lane-line); stroke-width: 3; fill: none; vector-effect: non-scaling-stroke; }
  ...
  .aw3-move.dim { opacity: 0.1; }
  .aw3-move.active { stroke-width: 4; opacity: 1; }
  .mv-eb { stroke: #ea7317; }
  .mv-wb { stroke: #dc2626; }
  .mv-nb { stroke: #2563eb; }
  .mv-sb { stroke: #16a34a; }
  ...
  .aw3-legend { display: flex; flex-wrap: wrap; gap: 0.4rem; margin-top: 0.35rem; }
  .aw3-chip {
    display: inline-flex;
    align-items: center;
    gap: 0.35rem;
  ...
  .aw3-chip.active { border-color: var(--diag-edge); }
