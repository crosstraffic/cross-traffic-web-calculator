...
export function discussion(results, inputs) {
  ...
  if (failing.length > 0 && totalDemand > 0) {
    out.push(
      `${failing.length} movement${failing.length === 1 ? ' runs' : 's run'} at LOS E or worse, carrying ${share(
        failing.reduce((a, m) => a + (Number(m.demand) || 0), 0),
        totalDemand,
        0,
      )}% of the ${n(totalDemand, 0)} veh/h the intersection letter is demand-weighted over.`,
    );
  }
