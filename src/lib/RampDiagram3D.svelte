  ...
  function build(lanes, isOn, onRight, nRamp, sclPx) {
    ...
    const sclSlab = isOn
      ? [
          [laneX0, ry(0)],
          [taperTip, ry(0)],
          [laneX1, ry(LANE * nRamp)],
          [laneX0, ry(LANE * nRamp)],
        ]
      : [
          [taperTip, ry(0)],
          [laneX1, ry(0)],
    ...
    const infl = [
      [inflX, inflY],
      [inflX + 170, inflY],
      [inflX + 170, inflY + LANE * inflLanes],
      [inflX, inflY + LANE * inflLanes],
    ];
    ...
    laneLines.push([
      [Math.min(laneX0, taperTip), edgeY],
      [Math.max(laneX1, taperTip), edgeY],
    ]);
    if (nRamp === 2) {
      laneLines.push([
        [laneX0, ry(LANE)],
...
<style>
  ...
  .swatch.ramp {
    background: var(--diag-scl-active-3d);
  }
  .swatch.influence {
    background: var(--diag-infl-soft);
    border: 1px solid var(--diag-infl-edge);
  }
</style>
