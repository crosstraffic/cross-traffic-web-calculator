...
function build(c) {
  return new m.WasmWeavingSegment(
    ...
    c.lc_fr,
    c.lc_rr,
    c.interchange_density,
    c.basic_freeway_capacity,
    c.caf,
    c.saf,
  );
}
...
// demands already in pc/h (PHF = 1.00, no heavy vehicles).
{
  ...
  approx(seg.get_volume_ratio(), 0.18, 0.001, 'EP2 VR');
  ...
  optCheck(
    ...
    (v) => approx(v, 13333.0, 15.0, 'EP2 c_W weaving (pc/h)'),
    'EP2 c_W weaving (pc/h)',
  );
...
// tolerances are widened to +-10 lc/h for this case (as in the Rust test).
{
  ...
  optCheck(
    seg,
    'get_capacity_weaving',
    (v) => exact(v, undefined, 'EP3 c_W weaving is None (two-sided)'),
    'EP3 c_W weaving is None (two-sided)',
  );
...
// curved part of Equation 12-1.
{
  ...
  approx(a.speed_avg, 70.7, 0.01, 'EP2 7.1 S_o (mi/h, p. 27-9)');
...
// The manual skips Step 4 here, so it prints no capacity and none is checked.
{
  ...
  approx(a.weaving_intensity, 0.00804, 5e-6, 'EP4-T1 W (p. 27-18)');
...
// --- Edition selection is a string, and a wrong one is silent.
{
  ...
  exact(
    Math.abs(v7.get_density() - v71.get_density()) > 1.0,
    true,
...
// (LOS A is D <= 11), so the flip density is the edge itself.
{
  ...
  const crossing = (letter) => {
    ...
    for (let i = 0; i < 60; i += 1) {
      const mid = (lo + hi) / 2;
      if (rank(scaled(mid).los) <= rank(letter)) lo = mid;
      else hi = mid;
    }
    return scaled(lo);
  };
