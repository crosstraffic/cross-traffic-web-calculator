...
function build({
  legacy = false,
  seeds = [cfg.weather_seed, cfg.demand_seed, cfg.incident_seed],
  strategies = [],
} = {}) {
  const rel = new m.WasmUrbanReliability(
    ...
    legacy ? undefined : c1.facility.prop_left_turn_lanes,
  ); // 1.0
  ...
  c1.facility.segments.forEach((s, i) => {
    ...
    rel.add_segment(
      ...
      legacy ? undefined : signal.approach_lanes,
    ); // 4
  });
  rel.run();
  return rel;
...
exact(
  r1.tti_percentile(50) <= r1.tti_percentile(80) && r1.tti_percentile(80) <= r1.tti_percentile(95),
  true,
  'EP4 percentile ordering',
);
...
exact(
  r1.num_oversaturated_scenarios(),
  70,
  'EP4 oversaturated scenarios (Rust test asserts >= 60; deterministic at seeds 82/11/63)',
);
...
exact(relDiff < 0.1, true, `replication: mean travel times agree within 10% (got ${(100 * relDiff).toFixed(2)}%)`);
...
const strategy1 = build({
  strategies: [{ name: 'EP5 Strategy 1: +5 s to the coordinated phase', effective_green_adjustment_s: 5.0 }],
});
exact(strategy1.num_scenarios(), 3120, 'EP5 Strategy 1 same reliability reporting period');
exact(
  strategy1.get_mean_travel_time() < r1.get_mean_travel_time(),
  true,
  `EP5 Strategy 1 reduces mean travel time (${strategy1.get_mean_travel_time().toFixed(1)} vs ${r1.get_mean_travel_time().toFixed(1)} s)`,
);
exact(
  strategy1.reliability_rating() >= r1.reliability_rating(),
  true,
...
exact(
  strategy1.num_oversaturated_scenarios() <= r1.num_oversaturated_scenarios(),
  true,
...
const asc = build({
  strategies: [{ name: 'Ch37 Sec.5 adaptive signal control (13.5% target)', sat_flow_adjustment: adaptiveSatFlow }],
});
approx(adaptiveSatFlow, 1.15607, 1e-5, 'adaptive signal control sat flow adjustment (Exhibit 37-9 midpoint)');
exact(
  asc.get_mean_travel_time() <= r1.get_mean_travel_time(),
  true,
...
exact(
  asc.reliability_rating() >= r1.reliability_rating(),
  true,
  'adaptive signal control does not degrade the reliability rating',
);
...
approx(
  legacy.reliability_rating(),
  95.0,
  5.0,
  'legacy call shape: reliability rating in [90, 100] vs published 93.2/94.1',
);
