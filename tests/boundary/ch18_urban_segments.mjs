...
function assertDelayIndependent(seg, tag) {
  ...
  approx(seg.get_through_delay(), 18.310, 0.001, `${tag} through delay pass-through [30-36]`);
  ...
  approx(seg.get_proportion_arriving_green(), 0.486, 0.001, `${tag} P = g/C (uniform arrivals; published dispersion value 0.493)`);
...
const segA = new m.WasmUrbanSegment(
  ...prefix(c1),
  Float64Array.from(c1.access_point_delays_s));  // [0.193, 0.194] s/veh (Exhibit 30-35)
...
approx(segB.get_travel_speed(), segA.get_travel_speed(), 0.005, 'case3 travel speed == case1 travel speed (computed vs supplied d_ap)');
...
const segC = new m.WasmUrbanSegment(
  ...
  c2.access_right_bay_adequate);    // false
...
approx(segC.get_access_point_delay(), 0.540, 0.005, 'case2 Sum d_ap (Exhibit 18-13 estimate; Section 4 procedure: 0.387)');
approx(segC.get_running_time(), 33.70, 0.01, 'case2 running time (computed)');
approx(segC.get_running_time(), 33.54, 0.5, 'case2 running time [30-36] within the planning-estimate tolerance');
approx(segC.get_travel_speed(), 23.60, 0.01, 'case2 travel speed (computed)');
...
approx(segLegacy.get_access_point_delay(), 2.96, 0.005, 'legacy default path d_ap (Exhibit 18-13 baseline, 0.37 s/pt x 8)');
