...
const shared = new m.WasmSharedUsePathPedestrian(
  sp.bicycle_demand_same_direction, sp.bicycle_demand_opposing, sp.phf,
  sp.pedestrian_speed, sp.bicycle_speed, sp.is_one_way);
...
const excl = new m.WasmExclusivePedestrianFacility(
  ep.total_walkway_width, ep.fixed_object_width, undefined, ep.peak_15min_volume,
  ep.phf, ep.pedestrian_speed, ep.facility_type.toLowerCase(), ep.flow_type.toLowerCase());
...
const bike = new m.WasmOffStreetBicycleFacility(
  bf.path_width, bf.segment_length, bf.has_centerline, bf.two_way_demand,
  bf.directional_split, bf.phf, bf.is_one_way, false, splits, speeds, sds);
