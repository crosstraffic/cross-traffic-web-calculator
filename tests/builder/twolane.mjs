...
import { deriveRows, deriveTwoLaneRows, PASSING_LANE_MIN_FT, TWOLANE_SEGMENT_KEYS } from '../../src/lib/builder/derive.js';
...
const grade = (id, aMi, bMi, gradePct, verticalClass = 1) => ({
	id, kind: 'grade', stationFt: aMi * MI, endFt: bMi * MI, label: '', gradePct, verticalClass
});
const passing = (id, aMi, bMi, passingType) => ({
	id, kind: 'passing', stationFt: aMi * MI, endFt: bMi * MI, label: '', passingType
});
const curve = (id, startFt, lengthFt, designRadiusFt, superelevationPct = 0) => ({
	id, kind: 'curve', stationFt: startFt, endFt: startFt + lengthFt, label: '',
	designRadiusFt, superelevationPct, centralAngleDeg: 0, horClassEntered: null
});
const demand = (id, stationFt, config) => ({
	id, kind: 'demand', stationFt, label: '',
...

{
	...
	eq(d.rows.map((r) => r.length), [1, 1, 2], 'a passing lane in the middle cuts the highway in three');
	eq(d.rows.map((r) => r.passing_type), [0, 2, 0], 'and only the covered stretch is the passing lane');
	eq(d.rows.map((r) => r.lanes), [1, 2, 1], 'the passing lane draws as the added lane it is');
	...
	ok(d.rows.every((r) => r.key.includes('ps1') || r.key.includes('start') || r.key.includes('end')),
...

{
	...
	const d = derive(doc);
	eq(d.rows.map((r) => r.passing_type), [0, 1, 0], 'a passing zone is type 1');
...

{
	...
	eq(ds.rows.map((r) => r.passing_type), [0, 0, 0], 'a 0.3 mi passing lane is analyzed as Passing Constrained');
	...
	eq(de.rows.map((r) => r.passing_type), [0, 2, 0], 'a passing lane exactly at the 0.5 mi minimum is NOT demoted');
...

{
	...
	const d = derive(doc);
	eq(d.rows.map((r) => r.grade), [0, 5, 0], 'a grade covers exactly the stretch it was placed over');
	eq(d.rows.map((r) => r.vertical_class), [1, 4, 1], 'and carries its vertical class with it');
	eq(d.rows.map((r) => r.length), [1, 1, 1], 'both of its ends are segment boundaries');
}

{
	...
	const d = derive(doc);
	eq(d.rows.map((r) => r.length), [2, 1], 'a demand change cuts the highway in two');
	eq(d.rows.map((r) => r.volume), [800, 1200], 'the upstream stretch takes the highway default');
	eq(d.rows.map((r) => r.phf), [0.95, 0.9], 'and the factors move with the volume');
	eq(d.rows.map((r) => r.phv), [5, 12], 'heavy vehicles is a PERCENT and is carried as one');
	eq(d.rows.map((r) => r.spl), [55, 55], 'a blank posted limit on the change inherits the highway\'s');
}

{
	...
	const d = derive(doc);
	eq(d.rows.map((r) => r.spl), [55, 45], 'a posted limit change reaches the segment below it');
...

{
	...
	eq(d.rows[0].subsegments.map((ss) => ss.length), [1000, 500, MI - 1500],
		'and the derivation fills tangents around it so the subsegments tile the segment');
	eq(d.rows[0].subsegments.map((ss) => ss.design_rad), [0, 600, 0], 'only the curve carries a radius');
	eq(d.rows[0].subsegments.map((ss) => ss.sup_ele), [0, 4, 0], 'superelevation is a PERCENT and rides with it');
	eq(d.rows[0].subsegments.map((ss) => ss.hor_class), [0, 0, 0],
		'the horizontal class is left at zero, because Step 5d computes it from the radius and overwrites anything supplied');
	near(d.rows[0].subsegments.reduce((a, ss) => a + ss.length, 0), d.rows[0].length_ft, 1e-6,
...

{
	...
	eq(df.rows[0].subsegments.map((ss) => ss.length), [MI], 'a curve filling its whole segment is one subsegment');
	ok(df.rows[0].subsegments.every((ss) => ss.length > 0), 'and no zero-length filler is emitted');

	const head = highway(1 * MI, [curve('hc1', 0, 500, 600, 4)]);
	eq(derive(head).rows[0].subsegments.map((ss) => ss.length), [500, MI - 500],
		'a curve at the start of a segment has no leading tangent');
	const tail = highway(1 * MI, [curve('hc1', MI - 500, 500, 600, 4)]);
	eq(derive(tail).rows[0].subsegments.map((ss) => ss.length), [MI - 500, 500],
...

{
	...
	eq(d.rows.map((r) => r.is_hc), [true, true], 'both segments are coded with horizontal curves');
	eq(d.rows[0].subsegments.map((ss) => [ss.length, ss.design_rad]), [[MI - 400, 0], [400, 500]],
		'the upstream half gets 400 ft of curve');
	eq(d.rows[1].subsegments.map((ss) => [ss.length, ss.design_rad]), [[400, 500], [MI - 400, 0]],
		'and the downstream half the other 400');
	for (const r of d.rows) {
		near(r.subsegments.reduce((a, ss) => a + ss.length, 0), r.length_ft, 1e-6,
...

{
	...
	const d = derive(doc);
	eq(d.rows[0].subsegments.map((ss) => ss.length), [500, 400, 600, 300, MI - 1800],
...

{
	...
	const d = derive(doc);
	eq(d.rows.map((r) => r.length), [4.1, 0.5, 0.5],
...

{
	...
	const EXPECT = {
		ch26ep1: {
			segs: 1,
			facilityFd: 10.092, facilityLos: 'D',
			ffs: [56.83], speed: [53.7], pf: [67.7], fd: [10.1], los: ['D']
		},
		ch26ep2: {
			segs: 1,
			facilityFd: 10.933, facilityLos: 'D',
			ffs: [56.83], speed: [49.5], pf: [67.7], fd: [10.9], los: ['D']
		},
		ch26ep3: {
			segs: 5,
			facilityFd: 7.271, facilityLos: 'C',
		...
		ch26ep4: {
			segs: 6,
			facilityFd: 19.897, facilityLos: 'E',
	...
	for (const [id, want] of Object.entries(EXPECT)) {
		...
		eq(blockingOf(doc).map((f) => f.id), [], `${id} has no blocking validation flags`);
		...
		for (let i = 0; i < fixture.segments.length; i++) {
			for (const k of ['passing_type', 'length', 'grade', 'spl', 'is_hc', 'volume', 'volume_op', 'vertical_class', 'phf', 'phv']) {
	...
	eq(TWOLANE_EXAMPLES.map((e) => e.id), ['ch26ep1', 'ch26ep2', 'ch26ep3', 'ch26ep4'], 'four examples are offered');
}

{
	...
	eq(dc.rows[0].subsegments.map((ss) => ss.length),
		[280, 432, 260, 366.5, 250, 216, 275.6, 458, 285, 767.9, 369],
		'at the published subsegment lengths, in feet');
	near(dc.rows[0].subsegments.reduce((a, ss) => a + ss.length, 0), 3960, 1e-6,
	...
	near(rc.segments[0].avgSpeed - rp.segments[0].avgSpeed, -4.13, 0.01,
	...
	near(rs.segments[0].avgSpeed, rp.segments[0].avgSpeed, 1e-9,
...

{
	...
	const d = derive(doc);
	eq(d.rows.map((r) => r.passing_type), [0, 0, 0, 0, 2, 0], 'the passing lane is derived as the fifth segment');
	const run = analyzeTwoLaneFacility(doc, d.rows, wasm);
	...
	ok(Math.abs(run.facilityFd - 14.936) > 1.0,
...

{
	...
	eq(run.segments.map((s) => [s.minLengthMi, s.maxLengthMi]),
		[[0.25, 3], [0.5, 3], [0.25, 3], [0.25, 2], [0.25, 3]],
		'the recommended bounds differ by passing type, which is what Exhibit 15-10 columns');
	eq(run.segments.map((s) => s.outsideRecommended), [false, false, false, false, false],
	...
	ok(twoLaneDiscussion(rShort).some((l) => /Exhibit 15-10 recommended length/.test(l)),
...

{
	for (const [i, name] of ['case1.json', 'case2.json', 'case3.json', 'case4.json'].entries()) {
		...
		const run = analyzeTwoLaneFacility(doc, d.rows, wasm);
		near(run.facilityFd, [10.092, 10.933, 7.271, 19.897][i], 0.0005,
...

{
	...
	eq(Object.keys(out.segments[0]).sort(), [...TWOLANE_SEGMENT_KEYS].sort(),
		'a built facility exports every key of the Chapter 15 segment schema');
	eq(Object.keys(out).filter((k) => k !== 'segments').sort(), ['apd', 'l_de', 'lane_width', 'pmhvfl', 'shoulder_width'],
...

{
	...
	eq(makeFeature(doc, 'curve', 1000).endFt - 1000, 400,
...

{
	...
	ok(df.errors.some((e) => /first segment is a passing lane/.test(e)),
	...
	const dt = derive(two);
	ok(dt.errors.some((e) => /2 passing lanes/.test(e)),
	...
	eq(blockingOf(one).map((f) => f.id), [], 'one passing lane, not first, is fine');
}

{
	...
	const d = derive(doc);
	ok(d.errors.some((e) => /overlap/.test(e)), 'overlapping passing features are reported');
	ok(d.rows.some((r) => r.passing_type === 2), 'and the overlap resolves to the more permissive of the two');
}

{
	...
	ok(flagIds(ffs).includes('spl-looks-like-ffs'),
...

{
	...
	ok(ids.includes('intersections-not-modelled'),
...

{
	...
	ok(lines.every((l) => typeof l === 'string' && l.length > 0), 'and none of them is empty');
	ok(lines.some((l) => l.includes('LOS E')), 'it names the facility letter');
	ok(lines.some((l) => /POSTED/.test(l) && /55/.test(l)),
		'it says the letter is read against the posted limit rather than the speed achieved');
	ok(lines.some((l) => /MIDPOINT/.test(l)), 'it says a passing lane\'s column is the midpoint value');
	ok(lines.some((l) => /Step 9/.test(l)), 'it names which segments the passing lane reached');
	ok(lines.some((l) => /subsegment/.test(l)), 'it says the curves are subsegments rather than rows');
	ok(lines.some((l) => /no travel time reliability/.test(l)),
	...
	ok(twoLaneDiscussion(rd).some((l) => /Exhibit 15-10 minimum/.test(l)),
...

{
	const doc = loadTwoLaneExample('ch26ep3');
	const run = analyzeTwoLaneFacility(doc, derive(doc).rows, wasm);
	eq(TWOLANE_MEASURES.map((m) => m.id), ['los', 'followerDensity', 'avgSpeed', 'percentFollowers', 'ffs'],
		'the selector offers the five measures a Chapter 15 result has');
	for (const m of TWOLANE_MEASURES) {
		ok(run.segments.every((s) => s[m.key] != null), `every segment carries ${m.id}, so no cell is blank`);
	...
	eq(cellText(fd, run.segments[0].followerDensity), run.segments[0].followerDensity.toFixed(2),
