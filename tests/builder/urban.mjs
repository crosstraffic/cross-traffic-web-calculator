...

{
  ...
  eq(
    derive(doc).rows.map((r) => r.cycle_length_s),
    [60, 140],
    'a segment takes its timing from its downstream signal',
  );
...

{
  ...
  eq(
    derive(doc).rows.map((r) => r.n_access_points_subject),
    [1, 0],
    'an access point on a boundary counts to the segment upstream of it',
  );
  ...
  eq(
    derive(doc2).rows.map((r) => r.n_access_points_subject),
    [1, 0],
    'an access point on the upstream terminus counts to the first segment',
  );
  ...
  eq(
    ...
    10,
    'every access point lands in exactly one segment, none dropped and none doubled',
  );
...

{
  ...
  eq(
    [r.n_access_points_subject, r.n_access_points_opposing],
    [1, 2],
    'subject and opposing access points are counted separately',
  );
}

{
  ...
  const planning = base();
  eq(
    derive(planning).rows[0].apDelaySource,
    'planning',
    'no delay and no approach falls to the Exhibit 18-13 planning estimate',
  );
  ...
  eq(
    derive(computed).rows[0].apDelaySource,
    'computed',
    'an approach alone takes the computed Chapter 30 Section 4 source',
  );
  ok(
    derive(computed).rows[0].analysis_period_h != null,
    'the computed source carries the analysis period the branch reads',
  );
  ...
  eq(
    derive(opp).rows[0].apDelaySource,
    'planning',
    'an opposing-side point supplies no delay however it is configured',
  );
}

{
  ...
  eq(
    derive(doc).rows.map((r) => r.speed_limit_mph),
    [35, 30],
    'a signal overrides the mainline speed limit for the segment ending at it',
  );
}

{
  ...
  eq(
    rows.map((r) => r.signal_spacing_ft),
    rows.map((r) => r.segment_length_ft),
    'signal spacing equals the derived segment length',
  );
}
...

{
  ...
  throws(
    () => migrate({ ...urban, facilityType: 'monorail' }),
    'unsupported facility type',
    'an unshipped facility type is refused',
  );
  eq(
    migrate({ ...emptyDocument('twolane') }).facilityType,
    'twolane',
    'and the two-lane type phase 3 shipped is accepted',
  );
}
...

{
  ...
  ok(
    flagIds(overCap).includes('demand-over-capacity'),
    'a through v/c above 1 is flagged, since it forces facility LOS F',
  );
  ...
  ok(
    flagIds(measures).includes('no-stop-rate'),
    'a summary segment missing its stop rate is a warning, since it leaves Equation 16-4 undefined',
  );
}
...

{
  ...
  eq(
    d.rows.map((r) => r.apDelaySource),
    ['published', 'published', 'published'],
    'its access points supply the Exhibit 30-35 per-point delays',
  );
  eq(
    d.rows.map((r) => [r.n_access_points_subject, r.n_access_points_opposing]),
    [
      [4, 4],
      [4, 4],
      [4, 4],
    ],
    'and N_ap,s = N_ap,o = 4 per segment',
  );
...

{
  ...
  near(
    ...
    23.67,
    0.01,
    'the computed Chapter 30 Section 4 procedure reproduces the same published travel speed',
  );
  near(
    compRun.travelSpeed,
    pubRun.travelSpeed,
    0.005,
    'computed and supplied agree to the rounding of the published per-point values they share',
  );
  ...
  ok(
    planRows[0].access_point_delays_s == null && planRows[0].access_point_approaches == null,
    'and sends neither of the other two',
  );
  ...
  ok(
    planRun.travelSpeed < pubRun.travelSpeed - 1.0,
    'and misses the published travel speed by over 1 mi/h, which is why the loaders supply a real source',
  );
  ...
  near(
    tunedRun.travelSpeed,
    23.6,
    0.05,
    "the Exhibit 18-13 parameters move the planning estimate to the ch18 boundary file's case2 value",
  );
  ok(
    tunedRun.travelSpeed > planRun.travelSpeed + 1.0,
    'which is over 1 mi/h above the same facility left at the exhibit defaults',
  );
  ...
  ok(
    bayRun.travelSpeed > planRun.travelSpeed,
    'two adequate turn bays take the Exhibit 18-13 delay to zero and raise the travel speed',
  );
...

{
  ...
  for (const [id, tag, speed, published, stopRate] of [
    ...
    near(
      ...
      published,
      tag === 'EB' ? 0.6 : 0.8,
      `Chapter 29 EP1 ${tag} travel speed within the published band [29-49]`,
    );
...

{
  ...
  near(
    ...
    base.travelSpeed,
    1e-9,
    'the effective green is inert when the through delay is supplied and the stop rate overridden',
  );
  near(
    green.spatialStopRate,
    base.spatialStopRate,
    1e-9,
    'and it does not move the stop rate either, since that is overridden too',
  );
...
for (const name of ['case1.json', 'case2.json', 'case3.json']) {
  ...
  eq(
    lengths(doc),
    raw.segments.map((s) => s.segment_length_ft),
    `${name} recovers the fixture's segment lengths`,
  );
...

{
  ...
  eq(
    fromUrbanFixture(loadCase('case1.json'), 'c1').analysisMode,
    'measures',
    'a fixture with published measures imports in measures mode',
  );
  eq(
    fromUrbanFixture(loadCase('case3.json'), 'c3').analysisMode,
    'inputs',
    'a fixture with Chapter 18 inputs imports in inputs mode',
  );
  ...
  ok(
    fromUrbanFixture(loadCase('case3.json'), 'c3').features[0].inferred,
    'the upstream terminus signal is marked as inferred',
  );
  // An imported urban fixture keeps its feature layer, unlike a freeway one.
  ok(
    fromUrbanFixture(loadCase('case3.json'), 'c3').importedSegments == null,
    'an urban import is not a segments-only document',
  );
...

{
  ...
  const other = analyzeUrbanReliability(
    doc,
    d.rows,
    { ...defaultUrbanReliabilityInputs(), weatherSeed: 83, demandSeed: 12, incidentSeed: 64 },
    defaultUrbanWeather(),
    wasm,
  );
...

{
  ...
  ok(
    !bareIds.includes('unbound-through_capacity_veh_h'),
    'a street not carrying a capacity is not told one was dropped',
  );
...

{
  ...
  ok(
    lines.some((l) => l.includes('%') && l.includes('base free-flow speed')),
    'it says the letter comes from the speed as a percentage of the base free-flow speed',
  );
  ok(
    lines.some((l) => l.includes('per-point values supplied')),
    'it names which access-point delay source was used',
  );
...

{
  for (const ex of URBAN_EXAMPLES) {
    ...
    eq(
      blocking.map((f) => f.id),
      [],
      `${ex.id} has no blocking validation flags`,
    );
  }
